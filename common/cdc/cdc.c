                   ...
                   gint64 *indexed)
{
    ...
    SyncwStat sb;
    if (syncw_fstat (fd_src, &sb) < 0) {
        syncw_warning ("CDC: failed to stat: %s.\n", strerror(errno));
        return -1;
    }
    ...
    while (1) {
        ...
        if (ret < 0) {
            syncw_warning ("CDC: failed to read: %s.\n", strerror(errno));
            free (buf);
            return -1;
        }
        tail += ret;
        ...
        if (file_descr->file_size > expected_size) {
            syncw_warning ("File size changed while chunking.\n");
            free (buf);
            return -1;
        }
        ...
        if (tail < block_min_sz || cur >= tail) {
            if (tail > 0) {
                if (file_descr->block_nr == file_descr->max_block_nr) {
                    syncw_warning ("Block id array is not large enough, bail out.\n");
                    free (buf);
                    return -1;
                }
...
int filename_chunk_cdc(const char *filename,
                       ...
                       gint64 *indexed)
{
    int fd_src = syncw_util_open (filename, O_RDONLY | O_BINARY);
    if (fd_src < 0) {
        syncw_warning ("CDC: failed to open %s.\n", filename);
        return -1;
    }
