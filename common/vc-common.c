...
static GList *
merge_bases_many (SyncwCommit *one, int n, SyncwCommit **twos);
...
static gboolean
add_to_commit_hash (SyncwCommit *commit, void *vhash, gboolean *stop)
{
...
static GHashTable *
commit_tree_to_hash (SyncwCommit *head)
{
    GHashTable *hash;
    gboolean res;
    ...
    res = syncw_commit_manager_traverse_commit_tree (syncw->commit_mgr,
                                                    head->repo_id,
                                                    head->version,
                                                    head->commit_id,
                                                    add_to_commit_hash,
        ...
        for (j = i+1; j < cnt; j++) {
            ...
            for (list = result; list; list = list->next) {
                ...
                if (strcmp (rslt[i]->commit_id, c->commit_id) == 0) {
                    syncw_commit_unref (rslt[i]);
                    rslt[i] = NULL;
                }
                if (strcmp (rslt[j]->commit_id, c->commit_id) == 0) {
                    syncw_commit_unref (rslt[j]);
                    rslt[j] = NULL;
                }
...
static gboolean
get_merge_bases (SyncwCommit *commit, void *vdata, gboolean *stop)
{
 ...
 */
SyncwCommit *
get_merge_base (SyncwCommit *head, SyncwCommit *remote)
{
    GList *result, *iter;
    SyncwCommit *one, **twos;
    int n, i;
    SyncwCommit *ret = NULL;

    one = head;
    twos = (SyncwCommit **) calloc (1, sizeof(SyncwCommit *));
    twos[0] = remote;
    n = 1;
    ...
    while (1) {
        ...
        one = result->data;
        twos = calloc (n, sizeof(SyncwCommit *));
...
{
    SyncwCommit *commit1, *commit2, *ca;
    VCCompareResult ret;
    ...
    commit1 = syncw_commit_manager_get_commit (syncw->commit_mgr, repo_id, version, c1);
    if (!commit1)
        return VC_INDEPENDENT;

    commit2 = syncw_commit_manager_get_commit (syncw->commit_mgr, repo_id, version, c2);
    if (!commit2) {
        syncw_commit_unref (commit1);
        return VC_INDEPENDENT;
    }
    ...
    if (ca) syncw_commit_unref (ca);
    syncw_commit_unref (commit1);
    syncw_commit_unref (commit2);
    return ret;
}
 ...
 */
static int
diff_parents_with_path (SyncwCommit *commit,
                        const char *repo_id,
                        const char *store_id,
                        int version,
                        const char *path,
                        ...
                        GError **error)
{
    ...
    if (commit->second_parent_id) {
        p2 = syncw_commit_manager_get_commit (syncw->commit_mgr,
                                             commit->repo_id,
                                             commit->version,
    ...
    if (!p2) {
        file_id_p1 = syncw_fs_manager_path_to_obj_id (syncw->fs_mgr,
                                                     store_id,
                                                     version,
                                                     p1->root_id, path,
                                                     NULL,
        ...
        if (*error)
            goto out;
        file_id_p2 = syncw_fs_manager_path_to_obj_id (syncw->fs_mgr,
                                                     store_id,
                                                     version,
                                                     p2->root_id, path,
                                                     NULL, error);
                            ...
                            char **modifier, gint64 *mtime)
{
    ...
    commit = syncw_commit_manager_get_commit (syncw->commit_mgr,
                                             ...
                                             head);
    if (!commit) {
        syncw_warning ("Failed to get commit %s.\n", head);
        return -1;
    }
