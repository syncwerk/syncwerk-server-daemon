...
static SyncwRiakClient *
get_connection (RiakPriv *priv)
{
    SyncwRiakClient *connection;

    pthread_mutex_lock (&priv->lock);

    connection = g_queue_pop_head (priv->conn_pool);
    if (!connection)
        connection = syncw_riak_client_new (priv->host, priv->port);
    pthread_mutex_unlock (&priv->lock);
    return connection;
}

static void
return_connection (RiakPriv *priv, SyncwRiakClient *connection)
{
    pthread_mutex_lock (&priv->lock);
...
obj_backend_riak_read (ObjBackend *bend,
                       ...
                       int *len)
{
    SyncwRiakClient *conn = get_connection (bend->priv);
    RiakPriv *priv = bend->priv;
    int ret;

    ret = syncw_riak_client_get (conn, priv->bucket, obj_id, data, len);
                        ...
                        int len)
{
    ...
    ret = syncw_riak_client_put (conn, priv->bucket, obj_id, data, len,
                                priv->n_write);
                         ...
                         const char *obj_id)
{
    ...
    ret = syncw_riak_client_query (conn, priv->bucket, obj_id);
                         ...
                         const char *obj_id)
{
    ...
    syncw_riak_client_delete (conn, priv->bucket, obj_id, priv->n_write);
...
obj_backend_riak_new (const char *host,
                      ...
                      const char *write_policy)
{
    syncw_warning ("Riak backend is not enabled.\n");
    return NULL;
}
