...
diff_entry_new_from_dirent (char type, char status,
                            SyncwDirent *dent, const char *basedir)
{
    DiffEntry *de = g_new0 (DiffEntry, 1);
...
dirent_same (SyncwDirent *denta, SyncwDirent *dentb)
{
...
static int
diff_files (int n, SyncwDirent *dents[], const char *basedir, DiffOptions *opt)
{
    SyncwDirent *files[3];
    int i, n_files = 0;
...
static int
diff_trees_recursive (int n, SyncwDir *trees[],
                      const char *basedir, DiffOptions *opt);

static int
diff_directories (int n, SyncwDirent *dents[], const char *basedir, DiffOptions *opt)
{
    SyncwDirent *dirs[3];
    int i, n_dirs = 0;
    char *dirname = "";
    int ret;
    SyncwDir *sub_dirs[3], *dir;
...
diff_trees (int n, const char *roots[], DiffOptions *opt)
{
    ...
    for (i = 0; i < n; ++i) {
        root = syncw_fs_manager_get_syncwdir (syncw->fs_mgr,
                                            opt->store_id,
                                            opt->version,
                                            roots[i]);
        if (!root) {
...
static int
twoway_diff_files (int n, const char *basedir, SyncwDirent *files[], void *vdata)
{
    DiffData *data = vdata;
    GList **results = data->results;
    DiffEntry *de;
    SyncwDirent *tree1 = files[0];
    SyncwDirent *tree2 = files[1];
...
static int
twoway_diff_dirs (int n, const char *basedir, SyncwDirent *dirs[], void *vdata,
                  gboolean *recurse)
{
    DiffData *data = vdata;
    GList **results = data->results;
    DiffEntry *de;
    SyncwDirent *tree1 = dirs[0];
    SyncwDirent *tree2 = dirs[1];
...
int
diff_commits (SyncwCommit *commit1, SyncwCommit *commit2, GList **results,
              gboolean fold_dir_diff)
{
    SyncwRepo *repo = NULL;
    DiffOptions opt;
    ...
    repo = syncw_repo_manager_get_repo (syncw->repo_mgr, commit1->repo_id);
    if (!repo) {
...
static int
threeway_diff_files (int n, const char *basedir, SyncwDirent *files[], void *vdata)
{
    DiffData *data = vdata;
    SyncwDirent *m = files[0];
    SyncwDirent *p1 = files[1];
    SyncwDirent *p2 = files[2];
    GList **results = data->results;
    DiffEntry *de;
...
static int
threeway_diff_dirs (int n, const char *basedir, SyncwDirent *dirs[], void *vdata,
                    gboolean *recurse)
{
    *recurse = TRUE;
    return 0;
...
int
diff_merge (SyncwCommit *merge, GList **results, gboolean fold_dir_diff)
{
    SyncwRepo *repo = NULL;
    DiffOptions opt;
    const char *roots[3];
    SyncwCommit *parent1, *parent2;
    ...
    repo = syncw_repo_manager_get_repo (syncw->repo_mgr, merge->repo_id);
    if (!repo) {
        syncw_warning ("Failed to get repo %s.\n", merge->repo_id);
        return -1;
    }

    parent1 = syncw_commit_manager_get_commit (syncw->commit_mgr,
                                              repo->id,
                                              repo->version,
                                              merge->parent_id);
    if (!parent1) {
    ...
    parent2 = syncw_commit_manager_get_commit (syncw->commit_mgr,
                                              repo->id,
                                              repo->version,
...
int diff_unmerged_state(int mask)
{
    ...
    switch (mask) {
        ...
        default:
            syncw_warning ("Unexpected unmerged case\n");
    }
    return 0;
}
