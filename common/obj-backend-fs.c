            ...
            const char *repo_id, int version)
{
    ...
    if (version > 0) {
        n = snprintf (path, SYNCW_PATH_MAX, "%s/%s/", priv->obj_dir, repo_id);
        pos += n;
    }
#else
    n = snprintf (path, SYNCW_PATH_MAX, "%s/%s/", priv->obj_dir, repo_id);
...
obj_backend_fs_read (ObjBackend *bend,
                     ...
                     int *len)
{
    char path[SYNCW_PATH_MAX];
    gsize tmp_len;
    GError *error = NULL;

    id_to_path (bend->priv, obj_id, path, repo_id, version);

    /* syncw_debug ("object path: %s\n", path); */
    ...
    if (error) {
        ...
        g_file_get_contents (path, (gchar**)data, &tmp_len, &error);
        if (error) {
            syncw_debug ("[obj backend] Failed to read object %s: %s.\n",
...
fsync_obj_contents (int fd)
{
    ...
    if (fsync (fd) < 0) {
        if (errno == EINVAL)
            ...
            syncw_warning ("Failed to fsync: %s.\n", strerror(errno));
            return -1;
        }
    }
    return 0;
    ...
    if (handle == INVALID_HANDLE_VALUE) {
        syncw_warning ("Failed to get handle from fd.\n");
        return -1;
    }

    if (!FlushFileBuffers (handle)) {
        syncw_warning ("FlushFileBuffer() failed: %lu.\n", GetLastError());
        return -1;
    }
...
rename_and_sync (const char *tmp_path, const char *obj_path)
{
    ...
    if (rename (tmp_path, obj_path) < 0) {
        syncw_warning ("Failed to rename from %s to %s: %s.\n",
    ...
    if (dir_fd < 0) {
        syncw_warning ("Failed to open dir %s: %s.\n", parent_dir, strerror(errno));
        goto out;
    }
    ...
    if (fsync (dir_fd) < 0) {
        if (errno != EINVAL) {
            syncw_warning ("Failed to fsync dir %s: %s.\n",
                          parent_dir, strerror(errno));
            ret = -1;
        }
        goto out;
...
out:
    ...
    if (!MoveFileExW (w_tmp_path, w_obj_path,
        ...
        syncw_warning ("MoveFilExW failed: %lu.\n", GetLastError());
        ret = -1;
        goto out;
    }
...
{
    char tmp_path[SYNCW_PATH_MAX];
    int fd;

    snprintf (tmp_path, SYNCW_PATH_MAX, "%s.XXXXXX", path);
    fd = g_mkstemp (tmp_path);
    if (fd < 0) {
        syncw_warning ("[obj backend] Failed to open tmp file %s: %s.\n",
                      tmp_path, strerror(errno));
        return -1;
    }

    if (writen (fd, data, len) < 0) {
        syncw_warning ("[obj backend] Failed to write obj %s: %s.\n",
                      tmp_path, strerror(errno));
        return -1;
    }
    ...
    if (close (fd) < 0) {
        syncw_warning ("[obj backend Failed close obj %s: %s.\n",
                      tmp_path, strerror(errno));
        return -1;
    }
    ...
    if (need_sync) {
        ...
        if (g_rename (tmp_path, path) < 0) {
            syncw_warning ("[obj backend] Failed to rename %s: %s.\n",
                          path, strerror(errno));
            return -1;
        }
    }
...
create_parent_path (const char *path)
{
    ...
    if (g_mkdir_with_parents (dir, 0777) < 0) {
        syncw_warning ("Failed to create object parent path %s: %s.\n",
                      dir, strerror(errno));
        g_free (dir);
        return -1;
    }
                      ...
                      gboolean need_sync)
{
    ...
    if (create_parent_path (path) < 0) {
        syncw_warning ("[obj backend] Failed to create path for obj %s:%s.\n",
                      repo_id, obj_id);
        return -1;
    }
    ...
    /* syncw_message ("write obj time: %ldus.\n", */
                            ...
                            void *user_data)
{
    ...
    while ((dname1 = g_dir_read_name(dir1)) != NULL) {
        ...
        if (!dir2) {
            syncw_warning ("Failed to open object dir %s.\n", path);
            continue;
        }
...
obj_backend_fs_copy (ObjBackend *bend,
                     ...
                     const char *obj_id)
{
    char src_path[SYNCW_PATH_MAX];
    char dst_path[SYNCW_PATH_MAX];
    ...
    if (create_parent_path (dst_path) < 0) {
        syncw_warning ("Failed to create dst path %s for obj %s.\n",
                      dst_path, obj_id);
        return -1;
    }
    ...
    if (!CreateHardLink (dst_path, src_path, NULL)) {
        syncw_warning ("Failed to link %s to %s: %lu.\n",
    ...
    if (ret < 0 && errno != EEXIST) {
        syncw_warning ("Failed to link %s to %s: %s.\n",
...
obj_backend_fs_remove_store (ObjBackend *bend, const char *store_id)
{
    ...
    while ((dname1 = g_dir_read_name(dir1)) != NULL) {
        ...
        if (!dir2) {
            syncw_warning ("Failed to open obj dir %s.\n", path1);
            g_dir_close (dir1);
            g_free (path1);
            g_free (obj_dir);
            return -1;
...
ObjBackend *
obj_backend_fs_new (const char *syncw_dir, const char *obj_type)
{
    ObjBackend *bend;
    FsPriv *priv;
    ...
    priv->obj_dir = g_build_filename (syncw_dir, "storage", obj_type, NULL);
    priv->dir_len = strlen (priv->obj_dir);

    if (g_mkdir_with_parents (priv->obj_dir, 0777) < 0) {
        syncw_warning ("[Obj Backend] Objects dir %s does not exist and"
                   " is unable to create\n", priv->obj_dir);
        goto onerror;
    }
