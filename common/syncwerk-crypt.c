...
{
    SyncwerkCrypt *crypt = g_new0 (SyncwerkCrypt, 1);
...
syncwerk_generate_random_key (const char *passwd, char *random_key)
{
    ...
    if (rc != 1) {
        syncw_warning ("Failed to generate secret key for repo encryption "
                      "with RAND_bytes(), use RAND_pseudo_bytes().\n");
        RAND_pseudo_bytes (secret_key, sizeof(secret_key));
    }

    syncwerk_derive_key (passwd, strlen(passwd), 2, key, iv);

    crypt = syncwerk_crypt_new (2, key, iv);

    syncwerk_encrypt ((char **)&rand_key, &outlen,
                        ...
                        const char *passwd, char *magic)
{
    ...
    syncwerk_derive_key (buf->str, buf->len, version, key, iv);
                            ...
                            int version)
{
    ...
    if (version != 1 && version != 2) {
        syncw_warning ("Unsupported enc_version %d.\n", version);
        return -1;
    }
                              ...
                              unsigned char *key_out, unsigned char *iv_out)
{
    ...
    syncwerk_derive_key (passwd, strlen(passwd), enc_version, key, iv);
    ...
    if (enc_version == 1) {
        ...
        if (random_key == NULL || random_key[0] == 0) {
            syncw_warning ("Empty random key.\n");
            return -1;
        }

        hex_to_rawdata (random_key, enc_random_key, 48);

        crypt = syncwerk_crypt_new (enc_version, key, iv);
        if (syncwerk_decrypt ((char **)&dec_random_key, &outlen,
                             (char *)enc_random_key, 48,
                             crypt) < 0) {
            syncw_warning ("Failed to decrypt random key.\n");
            g_free (crypt);
            return -1;
        }
        g_free (crypt);

        syncwerk_derive_key ((char *)dec_random_key, 32, enc_version,
                                  key, iv);
                           ...
                           const char *new_passwd, char *new_random_key)
{
    ...
    syncwerk_derive_key (old_passwd, strlen(old_passwd), 2, key, iv);

    hex_to_rawdata (old_random_key, random_key_raw, 48);

    crypt = syncwerk_crypt_new (2, key, iv);
    if (syncwerk_decrypt ((char **)&secret_key, &secret_key_len,
    ...
    syncwerk_derive_key (new_passwd, strlen(new_passwd), 2, key, iv);

    crypt = syncwerk_crypt_new (2, key, iv);

    syncwerk_encrypt ((char **)&new_random_key_raw, &random_key_len,
                 ...
                 SyncwerkCrypt *crypt)
{
    ...
    *data_out = (char *)g_malloc (blks * BLK_SIZE);
    ...
    if (*data_out == NULL) {
        syncw_warning ("failed to allocate the output buffer.\n");
        goto enc_error;
    }                
