...
create_parent_path (const char *path)
{
    ...
    if (g_mkdir_with_parents (dir, 0777) < 0) {
        syncw_warning ("Failed to create object parent path: %s.\n", dir);
        g_free (dir);
        return -1;
    }
                               ...
                               BHandle *handle)
{
    ...
    if (create_parent_path (path) < 0) {
        syncw_warning ("Failed to create path for block %s:%s.\n",
                      handle->store_id, handle->block_id);
        return -1;
    }

    if (g_rename (handle->tmp_file, path) < 0) {
        syncw_warning ("[block bend] failed to commit block %s:%s: %s\n",
...
block_backend_fs_block_exists (BlockBackend *bend,
                               ...
                               const char *block_sha1)
{
    char block_path[SYNCW_PATH_MAX];
                             ...
                             const char *block_id)
{
    ...
    if (syncw_stat (path, &st) < 0) {
        syncw_warning ("[block bend] Failed to stat block %s:%s at %s: %s.\n",
                                       ...
                                       BHandle *handle)
{
    ...
    if (syncw_fstat (handle->fd, &st) < 0) {
        syncw_warning ("[block bend] Failed to stat block %s:%s.\n",
                      handle->store_id, handle->block_id);
        return NULL;
    }
    block_md = g_new0(BMetadata, 1);
                                ...
                                void *user_data)
{
    ...
    while ((dname1 = g_dir_read_name(dir1)) != NULL) {
        ...
        if (!dir2) {
            syncw_warning ("Failed to open block dir %s.\n", path);
            continue;
        }
                       ...
                       const char *block_id)
{
    ...
    if (create_parent_path (dst_path) < 0) {
        syncw_warning ("Failed to create dst path %s for block %s.\n",
                      dst_path, block_id);
        return -1;
    }
...
#if defined MIGRATION
    if (version > 0) {
        n = snprintf (path, SYNCW_PATH_MAX, "%s/%s/", priv->block_dir, store_id);
        pos += n;
    } else
#else
    n = snprintf (path, SYNCW_PATH_MAX, "%s/%s/", priv->block_dir, store_id);
...
BlockBackend *
block_backend_fs_new (const char *syncw_dir, const char *tmp_dir)
{
    BlockBackend *bend;
    FsPriv *priv;
    ...
    priv->block_dir = g_build_filename (syncw_dir, "storage", "blocks", NULL);
    ...
    if (g_mkdir_with_parents (priv->block_dir, 0777) < 0) {
        syncw_warning ("Block dir %s does not exist and"
                   " is unable to create\n", priv->block_dir);
        goto onerror;
    }

    if (g_mkdir_with_parents (tmp_dir, 0777) < 0) {
        syncw_warning ("Blocks tmp dir %s does not exist and"
                   " is unable to create\n", tmp_dir);
        goto onerror;
    }
