...
def wait_for_server():
    syncwerk_api.get_repo_list(0, 1)
...
def encrypted_repo():
    ...
    try:
        syncwerk_api.post_dir(repo.id, '/', 'dir1', USER)
        syncwerk_api.post_dir(repo.id, '/', 'dir2', USER)
        syncwerk_api.post_dir(repo.id, '/dir1', 'subdir1', USER)
        syncwerk_api.post_dir(repo.id, '/dir2', 'subdir2', USER)
        yield repo
    finally:
        if syncwerk_api.get_repo(repo.id):
            # The repo may be deleted in the test case
            syncwerk_api.remove_repo(repo.id)
