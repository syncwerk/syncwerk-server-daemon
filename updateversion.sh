...
if test "$(uname)" = "Darwin"; then
    ...
    sed -i '' -e "s|<string>$old_ver</string>|<string>$new_ver</string>|" gui/mac/syncwerk/syncwerk/*.plist
else
    sed -i  "s|$old_ver|$new_ver|" web/setup_mac.py
    sed -i "s|VERSION=$old_ver|VERSION=$new_ver|" setupmac.sh
    sed -i "s|<string>$old_ver</string>|<string>$new_ver</string>|" gui/mac/syncwerk/syncwerk/*.plist
