...
checkdir (const char *dir)
{
    ...
    while (*p == '\\' || *p == '/') *p-- = '\0';
    if ((syncw_stat(dir, &st) < 0) || !S_ISDIR(st.st_mode)) {
        g_free (path);
        return -1;
    }
    g_free (path);
...
win32_83_path_to_long_path (const char *worktree, const wchar_t *path, int path_len)
{
    ...
    fullpath_long = g_new0 (wchar_t, SYNCW_PATH_MAX);

    DWORD n = GetLongPathNameW (fullpath_w, fullpath_long, SYNCW_PATH_MAX);
    if (n == 0) {
        /* Failed. */
...
int
syncw_stat (const char *path, SyncwStat *st)
{
#ifdef WIN32
...
int
syncw_fstat (int fd, SyncwStat *st)
{
#ifdef WIN32
    if (_fstat64 (fd, st) < 0)
        return -1;
...
void
syncw_stat_from_find_data (WIN32_FIND_DATAW *fdata, SyncwStat *st)
{
    memset (st, 0, sizeof(SyncwStat));
...
int
syncw_set_file_time (const char *path, guint64 mtime)
{
#ifndef WIN32
    struct stat st;
...
int
syncw_util_unlink (const char *path)
{
#ifdef WIN32
    wchar_t *wpath = win32_long_path (path);
    int ret = 0;
...
int
syncw_util_rmdir (const char *path)
{
#ifdef WIN32
    wchar_t *wpath = win32_long_path (path);
    int ret = 0;
...
int
syncw_util_mkdir (const char *path, mode_t mode)
{
#ifdef WIN32
    wchar_t *wpath = win32_long_path (path);
    int ret = 0;
...
int
syncw_util_open (const char *path, int flags)
{
#ifdef WIN32
    wchar_t *wpath;
    DWORD access = 0;
...
int
syncw_util_create (const char *path, int flags, mode_t mode)
{
#ifdef WIN32
    wchar_t *wpath;
    DWORD access = 0;
...
int
syncw_util_rename (const char *oldpath, const char *newpath)
{
#ifdef WIN32
...
gboolean
syncw_util_exists (const char *path)
{
#ifdef WIN32
    wchar_t *wpath = win32_long_path (path);
    DWORD attrs;
...
gint64
syncw_util_lseek (int fd, gint64 offset, int whence)
{
#ifdef WIN32
...
{
#ifdef WIN32
    char new_path[SYNCW_PATH_MAX + 1];
    char *p = new_path;
    const char *q = src;
    ...
    *next_out = '\0';
    ...
    if (*src == '~') {
        ...
        *next_out = '\0';
        ...
        getcwd (new_path, SYNCW_PATH_MAX);
...
get_process_handle (const char *process_name_in)
{
    ...
    HANDLE hProcess;
    HMODULE hMod;
    char process_name[SYNCW_PATH_MAX];
    unsigned int i;
...
find_process_in_dirent(struct dirent *dir, const char *process_name)
{
    ...
    ssize_t l = readlink(path, buf, SYNCW_PATH_MAX);
...
int
syncw_compress (guint8 *input, int inlen, guint8 **output, int *outlen)
{
    int ret;
    unsigned have;
    z_stream strm;
...
int
syncw_decompress (guint8 *input, int inlen, guint8 **output, int *outlen)
{
    int ret;
    unsigned have;
    z_stream strm;
...
char *
syncw_key_file_get_string (GKeyFile *key_file,
                          const char *group,
                          const char *key,
                          GError **error)
{
