...
send_handshake_response (BlockTxServer *server, int status)
{
    ...
    if (sendn (server->data_fd, &rsp, sizeof(rsp)) < 0) {
        syncw_warning ("Failed to send handshake response: %s.\n",
...
process_session_key (BlockTxServer *server, unsigned char *enc_session_key)
{
    ...
    client = ccnet_create_pooled_rpc_client (syncw->client_pool,
                                             ...
                                             "ccnet-rpcserver");
    if (!client) {
        syncw_warning ("Failed to create rpc client.\n");
    ...
    if (!key_b64) {
        syncw_warning ("Failed to decrypt session key.\n");
        send_handshake_response (server, STATUS_INTERNAL_SERVER_ERROR);
        ret = -1;
        goto out;
    }
...
handle_handshake_request (BlockTxServer *server)
{
    ...
    syncw_debug ("recv_state set to AUTH.\n");
...
send_auth_response (BlockTxServer *server, int status)
{
    ...
    if (send_encrypted_data_frame_begin (server->data_fd, sizeof(rsp)) < 0) {
        syncw_warning ("Send auth response: failed to begin.\n");
        ret = -1;
        goto out;
    }
    ...
    {
        syncw_warning ("Send auth response: failed to send data.\n");
        ret = -1;
        goto out;
    }

    if (send_encrypted_data_frame_end (ctx, server->data_fd) < 0) {
        syncw_warning ("Send auth response: failed to end.\n");
        ret = -1;
        goto out;
    }
...
handle_auth_req_content_cb (char *content, int clen, void *cbarg)
{
    ...
    if (session_token[clen - 1] != '\0') {
        syncw_warning ("Invalid session token format.\n");
    ...
    if (syncw_token_manager_verify_token (syncw->token_mgr, client, NULL,
                                         session_token, repo_id) < 0) {
        syncw_warning ("Session token check failed.\n");
    ...
    syncw_debug ("recv_state set to HEADER.\n");
...
send_block_response_header (BlockTxServer *server, int status)
{
    ...
    if (send_encrypted_data_frame_begin (server->data_fd, sizeof(header)) < 0) {
        syncw_warning ("Send block response header %s: failed to begin.\n",
    ...
    {
        syncw_warning ("Send block response header %s: failed to send data.\n",
                      server->curr_block_id);
        ret = -1;
        goto out;
    }
    ...
    if (send_encrypted_data_frame_end (ctx, server->data_fd) < 0) {
        syncw_warning ("Send block response header %s: failed to end.\n",
                      server->curr_block_id);
        ret = -1;
        goto out;
    }
...
handle_block_header_content_cb (char *content, int clen, void *cbarg)
{
    ...
    if (clen != sizeof(RequestHeader)) {
        syncw_warning ("Invalid block request header length %d.\n", clen);
    ...
    if (hdr->command != REQUEST_COMMAND_GET &&
        ...
        syncw_warning ("Unknow command %d.\n", hdr->command);
        send_block_response_header (server, STATUS_BAD_REQUEST);
        return -1;
    }
    ...
    if (server->command == REQUEST_COMMAND_GET) {
        ...
        syncw_debug ("Received GET request for block %s.\n", server->curr_block_id);

        md = syncw_block_manager_stat_block (syncw->block_mgr,
                                            server->store_id,
                                            server->repo_version,
                                            server->curr_block_id);
        if (!md) {
            syncw_warning ("Failed to stat block %s:%s.\n",
        ...
        syncw_debug ("Received PUT request for block %s.\n", server->curr_block_id);

        server->block = syncw_block_manager_open_block (syncw->block_mgr,
                                                       server->store_id,
                                                       server->repo_version,
                                                       server->curr_block_id,
                                                       BLOCK_WRITE);
        if (!server->block) {
            syncw_warning ("Failed to open block %s:%s for write.\n",
                          server->store_id, server->curr_block_id);
            send_block_response_header (server, STATUS_INTERNAL_SERVER_ERROR);
            return -1;
        }

        syncw_debug ("recv_state set to CONTENT.\n");
                      ...
                      int size)
{
    ...
    if (send_encrypted_data_frame_begin (server->data_fd, size) < 0) {
        syncw_warning ("Send block %s: failed to begin.\n", block_id);
        ret = -1;
        goto out;
    }
    ...
    remain = size;
    while (remain > 0) {
        n = syncw_block_manager_read_block (syncw->block_mgr,
                                           handle,
        ...
        if (send_encrypted_data (ctx, server->data_fd, send_buf, n) < 0) {
            syncw_warning ("Send block %s: failed to send data.\n", block_id);
            ret = -1;
            goto out;
        }
    ...
    if (send_encrypted_data_frame_end (ctx, server->data_fd) < 0) {
        syncw_warning ("Send block %s: failed to end.\n", block_id);
        ret = -1;
        goto out;
    }

    syncw_debug ("Send block %s done.\n", server->curr_block_id);
...
save_block_content_cb (char *content, int clen, int end, void *cbarg)
{
    ...
    n = syncw_block_manager_write_block (syncw->block_mgr, server->block,
                                        content, clen);
    if (n < 0) {
        syncw_warning ("Failed to write block %s.\n", server->curr_block_id);
        send_block_response_header (server, STATUS_INTERNAL_SERVER_ERROR);
        return -1;
    }

    if (end) {
        if (syncw_block_manager_close_block (syncw->block_mgr, server->block) < 0) {
            syncw_warning ("Failed to close block %s.\n", server->curr_block_id);
            send_block_response_header (server, STATUS_INTERNAL_SERVER_ERROR);
            return -1;
        }

        if (syncw_block_manager_commit_block (syncw->block_mgr, server->block) < 0) {
            syncw_warning ("Failed to commit block %s.\n", server->curr_block_id);
            send_block_response_header (server, STATUS_INTERNAL_SERVER_ERROR);
            return -1;
        }

        syncw_block_manager_block_handle_free (syncw->block_mgr, server->block);
        /* Set this handle to invalid. */
        server->block = NULL;

        send_block_response_header (server, STATUS_OK);

        syncw_debug ("Receive block %s done.\n", server->curr_block_id);
        syncw_debug ("recv_state set to HEADER.\n");
...
recv_data_cb (BlockTxServer *server)
{
    ...
    if (n == 0) {
        syncw_debug ("Data connection is closed by the client. Transfer done.\n");
        server->break_loop = TRUE;
        return;
    } else if (n < 0) {
        syncw_warning ("Read data connection error: %s.\n",
                      evutil_socket_error_to_string(evutil_socket_geterror(server->data_fd)));
        server->break_loop = TRUE;
        return;
    }
...
server_thread_loop (BlockTxServer *server)
{
    ...
    while (1) {
        ...
        if (rc < 0 && errno == EINTR) {
            ...
            syncw_warning ("select error: %s.\n", strerror(errno));
            break;
        }

        if (rc == 0) {
            syncw_warning ("Recv block timeout.\n");
            break;
        }
...
block_tx_server_thread (void *vdata)
{
    ...
    if (server->block) {
        syncw_block_manager_close_block (syncw->block_mgr, server->block);
        syncw_block_manager_block_handle_free (syncw->block_mgr, server->block);
    }
...
block_tx_server_start (evutil_socket_t data_fd)
{
    ...
    ret = ccnet_job_manager_schedule_job (syncw->job_mgr,
                                          block_tx_server_thread,
                                          block_tx_server_thread_done,
                                          server);
    if (ret < 0) {
        syncw_warning ("Failed to start block tx server thread.\n");
        return -1;
    }
