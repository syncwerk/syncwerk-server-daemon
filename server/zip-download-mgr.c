...
free_download_obj (DownloadObj *obj)
{
    ...
    g_free (obj->token);
    syncw_repo_unref (obj->repo);
    g_free (obj->user);
    g_free (obj->dir_name);
    if (obj->type == DOWNLOAD_DIR) {
        g_free ((char *)obj->internal);
    } else {
        g_list_free_full ((GList *)obj->internal, (GDestroyNotify)syncw_dirent_free);
    }
    g_free (obj);
}
...
zip_download_mgr_new ()
{
    ...
    if (!priv->zip_tpool) {
        if (error) {
            syncw_warning ("Failed to create zip task thread pool: %s.\n", error->message);
            g_clear_error (&error);
        } else {
            syncw_warning ("Failed to create zip task thread pool.\n");
        }
        g_free (priv);
        g_free (mgr);
        return NULL;
...
static SyncwerkCrypt *
get_syncwerk_crypt (SyncwRepo *repo, const char *user)
{
    SyncwerkCryptKey *key = NULL;
    char *key_hex, *iv_hex;
    unsigned char enc_key[32], enc_iv[16];
    SyncwerkCrypt *crypt = NULL;

    key = syncw_passwd_manager_get_decrypt_key (syncw->passwd_mgr,
                                               repo->id, user);
    if (!key) {
        syncw_warning ("Failed to get derypt key for repo %.8s.\n", repo->id);
        return NULL;
    }
    ...
    crypt = syncwerk_crypt_new (repo->enc_version, enc_key, enc_iv);
    g_free (key_hex);
    g_free (iv_hex);
    g_object_unref (key);
...
start_zip_task (gpointer data, gpointer user_data)
{
    ...
    SyncwRepo *repo = obj->repo;
    SyncwerkCrypt *crypt = NULL;
    int ret = 0;

    if (repo->encrypted) {
        crypt = get_syncwerk_crypt (repo, obj->user);
        if (!crypt) {
            ret = -1;
            goto out;
        }
...
parse_download_dir_data (DownloadObj *obj, const char *data)
{
    ...
    if (!jobj) {
        syncw_warning ("Failed to parse download dir data: %s.\n", jerror.text);
        return -1;
    }
    ...
    if (!dir_name || strcmp (dir_name, "") == 0) {
        syncw_warning ("Invalid download dir data: miss dir_name filed.\n");
        json_decref (jobj);
        return -1;
    }
    ...
    if (!obj_id || strcmp (obj_id, "") == 0) {
        syncw_warning ("Invalid download dir data: miss obj_id filed.\n");
        json_decref (jobj);
        return -1;
    }
...
parse_download_multi_data (DownloadObj *obj, const char *data)
{
    ...
    jobj = json_loadb (data, strlen(data), 0, &jerror);
    if (!jobj) {
        syncw_warning ("Failed to parse download multi data: %s.\n", jerror.text);
        return -1;
    }
    ...
    if (!tmp_parent_dir || strcmp (tmp_parent_dir, "") == 0) {
        syncw_warning ("Invalid download multi data, miss parent_dir field.\n");
        json_decref (jobj);
        return -1;
    }
    name_array = json_object_get (jobj, "file_list");
    if (!name_array) {
        syncw_warning ("Invalid download multi data, miss file_list field.\n");
        json_decref (jobj);
        return -1;
    }
    len = json_array_size (name_array);
    if (len == 0) {
        syncw_warning ("Invalid download multi data, miss download file name.\n");
        json_decref (jobj);
        return -1;
    }
    ...
    for (i = 0; i < len; i++) {
        ...
        if (strcmp (file_name, "") == 0 || strchr (file_name, '/') != NULL) {
            syncw_warning ("Invalid download file name: %s.\n", file_name);
            if (dirent_list) {
                g_list_free_full (dirent_list, (GDestroyNotify)syncw_dirent_free);
                dirent_list = NULL;
            }
            break;
        }
        ...
        dirent = syncw_fs_manager_get_dirent_by_path (syncw->fs_mgr, repo->store_id,
                                                     repo->version, repo->root_id,
                                                     file_path, &error);
        if (!dirent) {
            if (error) {
                syncw_warning ("Failed to get dirent for %s: %s.\n",
                              file_path, error->message);
                g_clear_error (&error);
            } else {
                syncw_warning ("Failed to get dirent for %s.\n",
                              file_path);
            }
            g_free (file_path);
            if (dirent_list) {
...
static gint64
calcuate_download_multi_size (SyncwRepo *repo, GList *dirent_list)
{
    ...
    for (; iter; iter = iter->next) {
        ...
        if (S_ISREG(dirent->mode)) {
            if (repo->version > 0) {
                ...
                size = syncw_fs_manager_get_file_size (syncw->fs_mgr, repo->store_id,
                                                      repo->version, dirent->id);
            }
            if (size < 0) {
                syncw_warning ("Failed to get file %s size.\n", dirent->name);
                return -1;
            }
            total_size += size;
        } else if (S_ISDIR(dirent->mode)) {
            size = syncw_fs_manager_get_fs_size (syncw->fs_mgr, repo->store_id,
                                                repo->version, dirent->id);
            if (size < 0) {
                syncw_warning ("Failed to get dir %s size.\n", dirent->name);
                return -1;
            }
            total_size += size;
        }
...
static int
calcuate_download_multi_file_count (SyncwRepo *repo, GList *dirent_list)
{
    GList *iter = dirent_list;
    SyncwDirent *dirent;
    int cur_count;
    ...
    for (; iter; iter = iter->next) {
        ...
        if (S_ISREG(dirent->mode)) {
            ...
            cur_count = syncw_fs_manager_count_fs_files (syncw->fs_mgr, repo->store_id,
                                                        repo->version, dirent->id);
            if (cur_count < 0) {
                syncw_warning ("Failed to get dir %s file count.\n", dirent->name);
                return -1;
            }
            count += cur_count;
        }
...
validate_download_size (DownloadObj *obj, GError **error)
{
    ...
    if (obj->type == DOWNLOAD_DIR) {
        download_size = syncw_fs_manager_get_fs_size (syncw->fs_mgr,
    ...
    /* default is MB */
    max_download_dir_size = syncw_cfg_manager_get_config_int64 (syncw->cfg_mgr, "fileserver",
    ...
    if (download_size < 0) {
        syncw_warning ("Failed to get download size.\n");
        ...
        syncw_warning ("Total download size %"G_GINT64_FORMAT
...
get_download_file_count (DownloadObj *obj, GError **error)
{
    ...
    if (obj->type == DOWNLOAD_DIR) {
        file_count = syncw_fs_manager_count_fs_files (syncw->fs_mgr, repo->store_id,
    ...
    if (file_count < 0) {
        syncw_warning ("Failed to get download file count.\n");
        g_set_error (error, SYNCWERK_DOMAIN, SYNCW_ERR_GENERAL,
                                 ...
                                 GError **error)
{
    ...
    repo_id = syncwerk_web_access_get_repo_id (info);
    ...
    data = syncwerk_web_access_get_obj_id (info);
    operation = syncwerk_web_access_get_op (info);

    obj = g_new0 (DownloadObj, 1);
    obj->token = g_strdup (token);
    obj->repo = repo;
    obj->user = g_strdup (syncwerk_web_access_get_username (info));
    ...
    if (strcmp (operation, "download-dir") == 0 ||
        ...
        if (!syncw_fs_manager_object_exists (syncw->fs_mgr,
                                            repo->store_id, repo->version,
                                            (char *)obj->internal)) {
            syncw_warning ("Dir %s doesn't exist.\n", (char *)obj->internal);
            g_set_error (error, SYNCWERK_DOMAIN, SYNCW_ERR_GENERAL,
                         "Dir doesn't exist.");
            ret = -1;
            goto out;
                                     ...
                                     const char *token, GError **error)
{
    ...
    if (!progress) {
        syncw_warning ("Zip progress info not found for token %s: "
