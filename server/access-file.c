...
{
    if (data->handle) {
        syncw_block_manager_close_block(syncw->block_mgr, data->handle);
        syncw_block_manager_block_handle_free(syncw->block_mgr, data->handle);
    }
...
free_senddir_data (SendDirData *data)
{
    ...
    zip_download_mgr_del_zip_progress (syncw->zip_download_mgr, data->token);
...
write_block_data_cb (struct bufferevent *bev, void *ctx)
{
    ...
    if (!data->handle) {
        data->handle = syncw_block_manager_open_block(syncw->block_mgr,
                                                     data->store_id,
                                                     data->repo_version,
                                                     blk_id, BLOCK_READ);
        if (!data->handle) {
    ...
    data->remain -= n;
    if (n < 0) {
        syncw_warning ("Error when reading from block %s:%s.\n",
                      data->store_id, blk_id);
        goto err;
    } else if (n == 0) {
...
next:
    ...
    if (!data->handle) {
        ...
        BlockMetadata *bmd;
        bmd = syncw_block_manager_stat_block_by_handle (syncw->block_mgr,
                                                       data->handle);
        if (!bmd)
            goto err;
        ...
        if (data->crypt) {
            if (syncwerk_decrypt_init (&data->ctx,
                                      data->crypt->version,
    ...
    if (data->crypt != NULL) {
        ...
        int ret = EVP_DecryptUpdate (data->ctx,
                                     ...
                                     n);
        if (ret == 0) {
            syncw_warning ("Decrypt block %s:%s failed.\n", data->store_id, blk_id);
            g_free (dec_out);
            goto err;
        }
...
write_dir_data_cb (struct bufferevent *bev, void *ctx)
{
    ...
    if (n < 0) {
        syncw_warning ("Failed to read zipfile %s: %s.\n", data->zipfile, strerror (errno));
        evhtp_connection_free (evhtp_request_get_connection (data->req));
...
static int
do_file(evhtp_request_t *req, SyncwRepo *repo, const char *file_id,
        const char *filename, const char *operation,
        SyncwerkCryptKey *crypt_key, const char *user)
{
    Syncwerk *file;
    char *type = NULL;
    char file_size[255];
    gchar *content_type = NULL;
    char cont_filename[SYNCW_PATH_MAX];
    char *key_hex, *iv_hex;
    ...
    if (strcmp(operation, "download") == 0 ||
        ...
        snprintf(cont_filename, SYNCW_PATH_MAX,
...
static BlockHandle *
get_start_block_handle (const char *store_id, int version, Syncwerk *file,
                        guint64 start, int *blk_idx)
{
    ...
    for (; i < file->n_blocks; i++) {
        ...
        bmd = syncw_block_manager_stat_block(syncw->block_mgr, store_id,
                                            version, blkid);
        if (!bmd)
            return NULL;
    ...
    if (start > tolsize) {
        ...
        int n = syncw_block_manager_read_block(syncw->block_mgr, handle,
                                              tmp, start-tolsize);
        if (n != start-tolsize) {
...
next:
    ...
    n = syncw_block_manager_read_block(syncw->block_mgr, data->handle, buf, bsize);
    data->range_remain -= n;
    if (n < 0) {
        syncw_warning ("Error when reading from block %s:%s.\n",
                      data->store_id, blk_id);
...
static int
do_file_range (evhtp_request_t *req, SyncwRepo *repo, const char *file_id,
                         ...
                         const char *repo_id, const char *user, const char *token_type)
{
    ...
    if (zipfd < 0) {
        syncw_warning ("Failed to open zipfile %s: %s.\n", zipfile, strerror(errno));
        return -1;
    }
...
access_zip_cb (evhtp_request_t *req, void *arg)
{
    ...
    token = parts[1];
    info = syncw_web_at_manager_query_access_token (syncw->web_at_mgr, token);
    ...
    if (!info) {
        syncw_warning ("Token doesn't exist.\n");
        error = "Invalid token\n";
        error_code = EVHTP_RES_BADREQ;
        goto out;
    }
    ...
    if (!info_obj) {
        syncw_warning ("Failed to parse obj_id field: %s.\n", jerror.text);
        error = "Invalid token\n";
        error_code = EVHTP_RES_BADREQ;
        goto out;
    }
    ...
    zip_file_path = zip_download_mgr_get_zip_file_path (syncw->zip_download_mgr, token);
    if (!zip_file_path) {
        g_object_get (info, "repo_id", &repo_id, NULL);
        syncw_warning ("Failed to get zip file path for %s in repo %.8s, token:[%s].\n",
    ...
    if (can_use_cached_content (req)) {
        ...
        zip_download_mgr_del_zip_progress (syncw->zip_download_mgr, token);
        goto out;
    }
...
access_cb(evhtp_request_t *req, void *arg)
{
    ...
    webaccess = syncw_web_at_manager_query_access_token (syncw->web_at_mgr, token);
    if (!webaccess) {
        error = "Bad access token";
        goto bad_req;
    }

    repo_id = syncwerk_web_access_get_repo_id (webaccess);
    data = syncwerk_web_access_get_obj_id (webaccess);
    operation = syncwerk_web_access_get_op (webaccess);
    user = syncwerk_web_access_get_username (webaccess);
...
static int
do_block(evhtp_request_t *req, SyncwRepo *repo, const char *user, const char *file_id,
         const char *blk_id)
{
    Syncwerk *file;
    uint32_t bsize;
    ...
    for (i = 0; i < file->n_blocks; i++) {
        if (memcmp(file->blk_sha1s[i], blk_id, 40) == 0) {
            BlockMetadata *bm = syncw_block_manager_stat_block (syncw->block_mgr,
                                                               repo->store_id,
                                                               repo->version,
                                                               blk_id);
            if (bm && bm->size >= 0) {
...
access_blks_cb(evhtp_request_t *req, void *arg)
{
    ...
    repo_id = syncwerk_web_access_get_repo_id (webaccess);
    id = syncwerk_web_access_get_obj_id (webaccess);
    operation = syncwerk_web_access_get_op (webaccess);
