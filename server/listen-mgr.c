...
struct _SyncwListenManagerPriv {
...
static int
get_listen_port (SyncwerkSession *session)
{
    char *port_str;
    int port = 0;
...
SyncwListenManager *
syncw_listen_manager_new (SyncwerkSession *session)
{
    SyncwListenManager *mgr;
    mgr = g_new0 (SyncwListenManager, 1);
    mgr->port = get_listen_port(session);

    mgr->priv = g_new0 (SyncwListenManagerPriv, 1);
...
syncw_listen_manager_start (SyncwListenManager *mgr)
{
    ...
    unsigned flags;
    SyncwListenManagerPriv *priv = mgr->priv;
    ...
    if (listenfd < 0) {
        syncw_warning ("[listen mgr] failed to bind port %d\n", mgr->port);
        return -1;
    }
    ...
    if (!priv->listener) {
        syncw_warning ("[listen mgr] failed to start evlistener\n");
    ...
    syncw_message ("listen on port %d for block tranfer\n", mgr->port);
    return 0;
}
...
read_cb (struct bufferevent *bufev, void *user_data)
{
    ...
    SyncwListenManager *mgr = user_data;
     ...
     * only be this length. */
    if (len != TOKEN_LEN) {
        syncw_warning ("[listen mgr] token with incorrect length received: %d\n",
                      (int)len);
        goto error;
    }
    ...
    if (strcmp (token, BLOCK_PROTOCOL_SIGNATURE) == 0) {
        if (ccnet_net_make_socket_blocking (connfd) < 0) {
            syncw_warning ("[listen mgr] Failed to set socket blocking.\n");
            goto error;
        }        
        if (block_tx_server_start (connfd) < 0) {
            syncw_warning ("Failed to start block tx server.\n");
            goto error;
        }
    ...
    if (!cbstruct) {
        syncw_warning ("[listen mgr] unknown token received: %s\n", token);
        goto error;
    }
...
error_cb (struct bufferevent *bufev, short what, void *user_data)
{
    if (what & BEV_EVENT_TIMEOUT)
        syncw_warning ("[listen mgr] client timeout\n");
    else
        syncw_warning ("[listen mgr] error when reading token\n");
...
is_token_expired (gpointer key, gpointer value, gpointer user_data)
{
    ...
    if (cbstruct->ttl == 0) {
        ...
        syncw_warning ("[listen mgr] token timeout\n");
...
{
    SyncwListenManager *mgr = vmanager;
