...
SizeScheduler *
size_scheduler_new (SyncwerkSession *session)
{
...
schedule_pulse (void *vscheduler)
{
    ...
    while (sched->priv->n_running_repo_size_jobs < CONCURRENT_JOBS) {
        ...
        if (now - sched->priv->last_print_queue_size_time >= PRINT_QUEUE_SIZE_INTERVAL) {
            syncw_message ("Repo size compute queue size is %u\n",
        ...
        int ret = ccnet_job_manager_schedule_job (sched->syncw->job_mgr,
                                                  compute_repo_size,
                                                  compute_repo_size_done,
                                                  job);
        if (ret < 0) {
            syncw_warning ("[scheduler] failed to start compute job.\n");
            g_free (job);
            break;
        }
...
static gboolean get_head_id (SyncwDBRow *row, void *data)
{
    char *head_id_out = data;
    const char *head_id;

    head_id = syncwerk_server_db_row_get_column_text (row, 0);
...
static int
set_repo_size_and_file_count (SyncwDB *db,
                              const char *repo_id,
                              ...
                              gint64 file_count)
{
    ...
    trans = syncwerk_server_db_begin_transaction (db);
    if (!trans)
        return -1;

    sql = "SELECT head_id FROM RepoSize WHERE repo_id=?";

    int n = syncwerk_server_db_trans_foreach_selected_row (trans, sql,
                                                get_head_id,
                                                cached_head_id,
                                                1, "string", repo_id);
    if (n < 0) {
    ...
    if (n == 0) {
        ...
        if (syncwerk_server_db_trans_query (trans, sql, 3, "string", repo_id, "int64", size,
        ...
        if (syncwerk_server_db_trans_query (trans, sql, 3, "int64", size, "string", new_head_id,
                                 "string", repo_id) < 0) {
            ret = -1;
            goto rollback;
        }
    ...
    exist = syncwerk_server_db_trans_check_for_existence (trans,
    ...
    if (exist) {
        if (syncwerk_server_db_trans_query (trans,
...
static char *
get_cached_head_id (SyncwDB *db, const char *repo_id)
{
    char *sql;

    sql = "SELECT head_id FROM RepoSize WHERE repo_id=?";
    return syncwerk_server_db_statement_get_string (db, sql, 1, "string", repo_id);
}
...
compute_repo_size (void *vjob)
{
    ...
    repo = syncw_repo_manager_get_repo (sched->syncw->repo_mgr, job->repo_id);
    if (!repo) {
        syncw_warning ("[scheduler] failed to get repo %s.\n", job->repo_id);
        return vjob;
    }

    cached_head_id = get_cached_head_id (sched->syncw->db, job->repo_id);
    if (g_strcmp0 (cached_head_id, repo->head->commit_id) == 0)
        goto out;

    head = syncw_commit_manager_get_commit (sched->syncw->commit_mgr,
                                           repo->id, repo->version,
                                           repo->head->commit_id);
    if (!head) {
        syncw_warning ("[scheduler] failed to get head commit %s.\n",
                   repo->head->commit_id);
        goto out;
    }

    size = syncw_fs_manager_get_fs_size (sched->syncw->fs_mgr,
                                        repo->store_id, repo->version,
                                        head->root_id);
    if (size < 0) {
        syncw_warning ("[scheduler] Failed to compute size of repo %.8s.\n",
                   repo->id);
        goto out;
    }

    file_count = syncw_fs_manager_count_fs_files (sched->syncw->fs_mgr,
                                                 repo->store_id, repo->version,
                                                 head->root_id);
    if (file_count < 0) {
        syncw_warning ("[scheduler] Failed to compute file count for repo %s.\n",
                      repo->id);
        goto out;
    }

    int ret = set_repo_size_and_file_count (sched->syncw->db,
                                            job->repo_id,
                                            repo->head->commit_id,
                                            size,
                                            file_count);
    if (ret < 0) {
        syncw_warning ("[scheduler] failed to store repo size and file count %s.\n", job->repo_id);
        goto out;
    }
