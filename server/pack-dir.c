                     ...
                     SyncwDirent *dent)
{
    ...
    struct SyncwerkCrypt *crypt = data->crypt;
    ...
    if (is_windows && syncw->http_server->windows_encoding) {
        char *win_file_name = do_iconv ("UTF-8",
                                        syncw->http_server->windows_encoding,
                                        pathname);
        if (!win_file_name) {
            syncw_warning ("Failed to convert file name to %s\n",
                          syncw->http_server->windows_encoding);
            ret = -1;
            goto out;
        }
    ...
    if (n != ARCHIVE_OK) {
        syncw_warning ("archive_write_header  error: %s\n", archive_error_string(a));
        ret = -1;
        goto out;
    }
    ...
    while (idx < file->n_blocks) {
        ...
        while (remain != 0) {
            ...
            if (crypt == NULL) {
                ...
                if (len <= 0) {
                    syncw_warning ("archive_write_data error: %s\n", archive_error_string(a));
                    ret = -1;
                    goto out;
                }
                ...
                if (r != 1) {
                    syncw_warning ("Decrypt block %s failed.\n", blk_id);
                    ret = -1;
                    goto out;
                }
             ...
             Progress *progress)
{
    ...
    dir = syncw_fs_manager_get_syncwdir (syncw->fs_mgr,
                                       ...
                                       root_id);
    if (!dir) {
        syncw_warning ("failed to get dir %s:%s\n", data->store_id, root_id);
        goto out;
    }
    if (!dir->entries) {
                   ...
                   gboolean is_windows)
{
    ...
    tmpfile_name = g_strdup_printf ("%s/syncwerk-XXXXXX.zip",
                                    syncw->http_server->http_temp_dir);
    fd = g_mkstemp (tmpfile_name);
    if (fd < 0) {
        syncw_warning ("Failed to open temp file: %s.\n", strerror (errno));
        g_free (tmpfile_name);
        return NULL;
    }
            ...
            Progress *progress)
{
    ...
    data = pack_dir_data_new (store_id, repo_version, dirname,
                              crypt, is_windows);
    if (!data) {
        syncw_warning ("Failed to create pack dir data for %s.\n",
    ...
    if (strcmp (dirname, "") != 0) {
        ...
        if (archive_dir (data, (char *)internal, "", progress) < 0) {
            if (progress->canceled)
                syncw_warning ("Zip task for dir %s in repo %.8s canceled.\n", dirname, store_id);
            else
                syncw_warning ("Failed to archive dir %s in repo %.8s.\n", dirname, store_id);
            ret = -1;
        }
    } else {
        // Pack multi
        if (archive_multi (data, (GList *)internal, progress) < 0) {
            if (progress->canceled)
                syncw_warning ("Archiving multi files in repo %.8s canceled.\n", store_id);
            else
                syncw_warning ("Failed to archive multi files in repo %.8s.\n", store_id);
            ret = -1;
        }
    }

    if (archive_write_finish(data->a) < 0) {
        syncw_warning ("Failed to archive write finish for %s in repo %.8s.\n",
