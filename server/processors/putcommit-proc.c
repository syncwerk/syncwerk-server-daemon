...
#define GET_PRIV(o)  \
   (G_TYPE_INSTANCE_GET_PRIVATE ((o), SYNCWERK_TYPE_PUTCOMMIT_PROC, SyncwerkPutcommitProcPriv))
...
G_DEFINE_TYPE (SyncwerkPutcommitProc, syncwerk_putcommit_proc, CCNET_TYPE_PROCESSOR)

static void
syncwerk_putcommit_proc_class_init (SyncwerkPutcommitProcClass *klass)
{
...
static void
syncwerk_putcommit_proc_init (SyncwerkPutcommitProc *processor)
{
}
...
static gboolean
commit_collector (SyncwCommit *commit, void *data, gboolean *stop)
{
    ObjectList *ol = data;
...
send_commit_ids (CcnetProcessor *processor, const char *head)
{
    ...
    if (ret == FALSE) {
        object_list_free (ol);
        syncw_warning ("[putcommit] Load commits error\n");
...
send_commit (CcnetProcessor *processor, char *object_id)
{
    ...
    if (syncw_obj_store_read_obj (syncw->commit_mgr->obj_store,
                                 object_id, (void**)&data, &len) < 0) {
        syncw_warning ("Failed to read commit %s.\n", object_id);
        goto fail;
    }
...
send_commits (CcnetProcessor *processor, char *content, int clen)
{
    ...
    if (clen % 41 != 1 || content[clen-1] != '\0') {
        syncw_warning ("[putcommit] Bad commit object list.\n");
                           ...
                           char *content, int clen)
{
    if (strncmp(code, SC_GET_OBJECT, 3) == 0) {
        ...
        syncw_warning ("[putcommit] Bad response: %s %s\n", code, code_msg);
        ccnet_processor_done (processor, FALSE);
    }
}
