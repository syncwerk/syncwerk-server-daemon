...
typedef struct {
    ...
    char root_id[41];
    int result;
} SyncwerkCheckffProcPriv;

#define GET_PRIV(o)  \
   (G_TYPE_INSTANCE_GET_PRIVATE ((o), SYNCWERK_TYPE_CHECKFF_PROC, SyncwerkCheckffProcPriv))

#define USE_PRIV \
    SyncwerkCheckffProcPriv *priv = GET_PRIV(processor);

G_DEFINE_TYPE (SyncwerkCheckffProc, syncwerk_checkff_proc, CCNET_TYPE_PROCESSOR)
...
release_resource(CcnetProcessor *processor)
{
    ...
    CCNET_PROCESSOR_CLASS (syncwerk_checkff_proc_parent_class)->release_resource (processor);
}


static void
syncwerk_checkff_proc_class_init (SyncwerkCheckffProcClass *klass)
{
    ...
    g_type_class_add_private (klass, sizeof (SyncwerkCheckffProcPriv));
}

static void
syncwerk_checkff_proc_init (SyncwerkCheckffProc *processor)
{
}
...
static gboolean
compare_root (SyncwCommit *commit, void *data, gboolean *stop)
{
    CompareAux *aux = data;
...
static gboolean
check_fast_forward (SyncwRepo *repo, const char *head_id, const char *root_id)
{
