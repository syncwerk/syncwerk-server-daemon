...
typedef struct {
    ...
    char *orig_path;
} SyncwerkCheckTxSlaveV3ProcPriv;

#define GET_PRIV(o)  \
   (G_TYPE_INSTANCE_GET_PRIVATE ((o), SYNCWERK_TYPE_CHECK_TX_SLAVE_V3_PROC, SyncwerkCheckTxSlaveV3ProcPriv))

#define USE_PRIV \
    SyncwerkCheckTxSlaveV3ProcPriv *priv = GET_PRIV(processor);

G_DEFINE_TYPE (SyncwerkCheckTxSlaveV3Proc, syncwerk_check_tx_slave_v3_proc, CCNET_TYPE_PROCESSOR)
...
release_resource(CcnetProcessor *processor)
{
    ...
    CCNET_PROCESSOR_CLASS (syncwerk_check_tx_slave_v3_proc_parent_class)->release_resource (processor);
}


static void
syncwerk_check_tx_slave_v3_proc_class_init (SyncwerkCheckTxSlaveV3ProcClass *klass)
{
    ...
    g_type_class_add_private (klass, sizeof (SyncwerkCheckTxSlaveV3ProcPriv));
}

static void
syncwerk_check_tx_slave_v3_proc_init (SyncwerkCheckTxSlaveV3Proc *processor)
{
}
...
decrypt_token (CcnetProcessor *processor)
{
    ...
    if (hex_len % 2 != 0) {
        syncw_warning ("[check tx slave v3] invalid length of encrypted token\n"); 
        ret = -1;
        goto out;
    }
    ...
    if (syncwerk_decrypt (&token, &token_len, encrypted_token,
        ...
        syncw_warning ("[check tx slave v3] failed to decrypt token\n");
        ret = -1;
        goto out;
    }
...
check_tx (void *vprocessor)
{
    ...
    if (repo->version > 0 && priv->client_version < 6) {
        syncw_warning ("Client protocol version is %d, "
...
start (CcnetProcessor *processor, int argc, char **argv)
{
    ...
    if (client_version < 5) {
        syncw_debug ("Client protocol version lower than 5, not supported.\n");
        ccnet_processor_send_response (processor,
    ...
    if (!peer || !peer->session_key) {
        syncw_warning ("[check tx slave v3] session key of peer %.10s is null\n",
                      processor->peer_id);
