...
G_DEFINE_TYPE (SyncwerkRecvbranchV2Proc, syncwerk_recvbranch_v2_proc, CCNET_TYPE_PROCESSOR)

#define GET_PRIV(o)  \
   (G_TYPE_INSTANCE_GET_PRIVATE ((o), SYNCWERK_TYPE_RECVBRANCH_V2_PROC, SyncwerkRecvbranchProcPriv))
...
release_resource(CcnetProcessor *processor)
{
    ...
    CCNET_PROCESSOR_CLASS (syncwerk_recvbranch_v2_proc_parent_class)->release_resource (processor);
}


static void
syncwerk_recvbranch_v2_proc_class_init (SyncwerkRecvbranchV2ProcClass *klass)
{
...
static void
syncwerk_recvbranch_v2_proc_init (SyncwerkRecvbranchV2Proc *processor)
{
}
...
retry:
    current_head = syncw_commit_manager_get_commit (syncw->commit_mgr,
                                                   ...
                                                   repo->head->commit_id);
    if (!current_head) {
        syncw_warning ("Failed to find head commit of %s:%s.\n",
