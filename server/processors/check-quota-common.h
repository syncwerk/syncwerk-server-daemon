...
static void
free_sync_rpc_client (RpcsyncwerkClient *rpc_client)
{
                        ...
                        const char *repo_id)
{
    ...
    user = syncw_repo_manager_get_repo_owner (syncw->repo_mgr, repo_id);
    if (user != NULL) {
        quota = syncw_quota_manager_get_user_quota (syncw->quota_mgr, user);
        if (quota <= 0)
            quota = syncw->quota_mgr->default_quota;
    } else {
        ...
        quota = syncw_quota_manager_get_org_quota (syncw->quota_mgr, org_id);
        if (quota <= 0)
            quota = syncw->quota_mgr->default_quota;
    }
    ...
    if (user)
        usage = syncw_quota_manager_get_user_usage (syncw->quota_mgr, user);
    else
        usage = syncw_quota_manager_get_org_usage (syncw->quota_mgr, org_id);
