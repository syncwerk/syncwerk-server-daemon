...
typedef struct  {
    ...
    char        repo_id[37];
    int         repo_version;
} SyncwerkPutcommitProcPriv;

#define GET_PRIV(o)  \
   (G_TYPE_INSTANCE_GET_PRIVATE ((o), SYNCWERK_TYPE_PUTCOMMIT_V2_PROC, SyncwerkPutcommitProcPriv))

#define USE_PRIV \
    SyncwerkPutcommitProcPriv *priv = GET_PRIV(processor);
...
G_DEFINE_TYPE (SyncwerkPutcommitV2Proc, syncwerk_putcommit_v2_proc, CCNET_TYPE_PROCESSOR)
...
release_resource (CcnetProcessor *processor)
{
    ...
    if (priv->registered)
        syncw_obj_store_unregister_async_read (syncw->commit_mgr->obj_store,
                                              priv->reader_id);

    CCNET_PROCESSOR_CLASS (syncwerk_putcommit_v2_proc_parent_class)->release_resource (processor);
}

static void
syncwerk_putcommit_v2_proc_class_init (SyncwerkPutcommitV2ProcClass *klass)
{
    ...
    g_type_class_add_private (klass, sizeof (SyncwerkPutcommitProcPriv));
}

static void
syncwerk_putcommit_v2_proc_init (SyncwerkPutcommitV2Proc *processor)
{
}
...
read_and_send_commit (CcnetProcessor *processor)
{
    ...
    if (syncw_obj_store_async_read (syncw->commit_mgr->obj_store,
                                   priv->reader_id,
                                   id) < 0) {
        syncw_warning ("[putcommit] Failed to start read of %s.\n", id);
...
read_done_cb (OSAsyncResult *res, void *cb_data)
{
    ...
    if (!res->success) {
        syncw_warning ("[putcommit] Failed to read %s.\n", res->obj_id);
        goto bad;
    }

    send_commit (processor, res->obj_id, res->data, res->len);

    syncw_debug ("Send commit %.8s.\n", res->obj_id);
...
static gboolean
collect_id_fast_forward (SyncwCommit *commit, void *data, gboolean *stop)
{
...
static gboolean
collect_id_remote (SyncwCommit *commit, void *data, gboolean *stop)
{
    CcnetProcessor *processor = data;
    USE_PRIV;
    char *key;
...
static gboolean
compute_delta (SyncwCommit *commit, void *data, gboolean *stop)
{
    CcnetProcessor *processor = data;
    USE_PRIV;
...
compute_delta_commits (CcnetProcessor *processor, const char *head)
{
    ...
    ret = syncw_commit_manager_traverse_commit_tree (syncw->commit_mgr,
                                                    priv->repo_id,
                                                    priv->repo_version,
                                                    ...
                                                    FALSE);
    if (!ret) {
        syncw_warning ("[putcommit] Failed to traverse remote branch.\n");
        string_list_free (priv->id_list);
        priv->id_list = NULL;
        return -1;
    }
    ...
    ret = syncw_commit_manager_traverse_commit_tree (syncw->commit_mgr,
                                                    ...
                                                    FALSE);
    if (!ret) {
        syncw_warning ("[putcommit] Failed to compute delta commits.\n");
        string_list_free (priv->id_list);
        priv->id_list = NULL;
        return -1;
    }
...
collect_commit_id_thread (void *vprocessor)
{
    ...
    if (syncw_commit_manager_traverse_commit_tree (syncw->commit_mgr,
                                                  repo->id,
                                                  repo->version,
                                                  priv->head_commit_id,
                                                  ...
                                                  processor,
                                                  FALSE) < 0) {
        syncw_warning ("[putcommit] Failed to collect commit id.\n");
        string_list_free (priv->id_list);
        priv->id_list = NULL;
        goto out;
    }
    ...
    if (priv->fast_forward) {
        syncw_debug ("Send commits after a fast-forward merge.\n");
        goto out;
    }

    syncw_debug ("Send commits after a real merge.\n");
...
collect_commit_id_done (void *vprocessor)
{
    ...
    priv->reader_id =
        syncw_obj_store_register_async_read (syncw->commit_mgr->obj_store,
                                            priv->repo_id,
                                            priv->repo_version,
                                            read_done_cb,
                                            processor);
