...
#define GET_PRIV(o)  \
   (G_TYPE_INSTANCE_GET_PRIVATE ((o), SYNCWERK_TYPE_PUTFS_PROC, PutfsProcPriv))

#define USE_PRIV \
    PutfsProcPriv *priv = GET_PRIV(processor);

G_DEFINE_TYPE (SyncwerkPutfsProc, syncwerk_putfs_proc, CCNET_TYPE_PROCESSOR)
...
release_resource(CcnetProcessor *processor)
{
    ...
    CCNET_PROCESSOR_CLASS (syncwerk_putfs_proc_parent_class)->release_resource (processor);
}


static void
syncwerk_putfs_proc_class_init (SyncwerkPutfsProcClass *klass)
{
...
static void
syncwerk_putfs_proc_init (SyncwerkPutfsProc *processor)
{
}
...
get_repo_info_done (void *data)
{
    ...
    if (priv->success) {
        ...
        priv->reader_id =
            syncw_obj_store_register_async_read (syncw->fs_mgr->obj_store,
                                                priv->store_id,
                                                priv->repo_version,
                                                read_done_cb,
                                                processor);
...
read_done_cb (OSAsyncResult *res, void *cb_data)
{
    ...
    if (!res->success) {
        syncw_warning ("[putfs] Failed to read %s.\n", res->obj_id);
    ...
    if (pack_size <= MAX_OBJ_SEG_SIZE) {
        ...
        while (offset < pack_size) {
            ...
            syncw_debug ("[putfs] Sent object %s segment<total = %d, offset = %d, n = %d>\n",
...
send_fs_object (CcnetProcessor *processor, char *object_id)
{
    ...
    if (syncw_obj_store_async_read (syncw->fs_mgr->obj_store,
                                   priv->reader_id,
                                   object_id) < 0) {
        syncw_warning ("[putfs] Failed to start async read of %s.\n", object_id);
...
send_fs_objects (CcnetProcessor *processor, char *content, int clen)
{
    ...
    if (clen % 41 != 1 || content[clen-1] != '\0') {
        syncw_warning ("[putfs] Bad fs object list.\n");
               ...
               char *content, int clen)
{
    if (strncmp(code, SC_GET_OBJECT, 3) == 0) {
        ...
        syncw_warning ("[putfs] Bad update: %s %s\n", code, code_msg);
        ccnet_processor_send_response (processor,
