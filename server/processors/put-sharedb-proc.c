...
#define GET_PRIV(o)  \
   (G_TYPE_INSTANCE_GET_PRIVATE ((o), SYNCWERK_TYPE_PUT_SHAREDB_PROC, PutSharedbPriv))
...
send_share_info (CcnetProcessor *processor)
{
    ...
    SyncwShareInfo *info = priv->ptr->data;
    char *s = syncw_share_info_to_json(info);
...
prepare_put_share_db (CcnetProcessor *processor)
{
    ...
    priv->sinfos = syncw_share_manager_get_group_share_info (
        syncw->share_mgr, priv->share_id);
