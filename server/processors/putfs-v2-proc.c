...
} SyncwerkPutfsProcPriv;

#define GET_PRIV(o)  \
   (G_TYPE_INSTANCE_GET_PRIVATE ((o), SYNCWERK_TYPE_PUTFS_V2_PROC, SyncwerkPutfsProcPriv))

#define USE_PRIV \
    SyncwerkPutfsProcPriv *priv = GET_PRIV(processor);

G_DEFINE_TYPE (SyncwerkPutfsV2Proc, syncwerk_putfs_v2_proc, CCNET_TYPE_PROCESSOR)
...
release_resource(CcnetProcessor *processor)
{
    ...
    CCNET_PROCESSOR_CLASS (syncwerk_putfs_v2_proc_parent_class)->release_resource (processor);
}


static void
syncwerk_putfs_v2_proc_class_init (SyncwerkPutfsV2ProcClass *klass)
{
    ...
    g_type_class_add_private (klass, sizeof(SyncwerkPutfsProcPriv));
}

static void
syncwerk_putfs_v2_proc_init (SyncwerkPutfsV2Proc *processor)
{
}
...
calculate_send_object_list_done (void *vdata)
{
    ...
    if (priv->send_obj_list == NULL) {
        syncw_message ("No fs objects to put. Done.\n");
                ...
                const char *object_id, char *data, int len)
{
    ...
    if (pack_size <= MAX_OBJ_SEG_SIZE) {
        ...
        while (offset < pack_size) {
            ...
            syncw_debug ("Sent object %s segment<total = %d, offset = %d, n = %d>\n",
...
fs_object_read_cb (OSAsyncResult *res, void *data)
{
    ...
    if (!res->success) {
        syncw_warning ("Failed to read fs object %.8s.\n", res->obj_id);
...
read_fs_object (CcnetProcessor *processor, const char *obj_id)
{
    ...
    syncw_obj_store_async_read (syncw->fs_mgr->obj_store,
                               priv->reader_id,
                               obj_id);
}
...
process_object_list_segment (CcnetProcessor *processor, char *content, int clen)
{
    ...
    syncw_debug ("%d objects are needed by the client.\n", n);
               ...
               char *content, int clen)
{
    ...
    case SEND_OBJECTS:
        if (strncmp (code, SC_OBJ_LIST_SEG, 3) == 0) {
            ...
            syncw_debug ("All objects received. Done.\n");
            ccnet_processor_done (processor, TRUE);
            return;
        }
        break;
