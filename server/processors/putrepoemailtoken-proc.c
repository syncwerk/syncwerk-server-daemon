...
typedef struct  {
    ...
    char *token;
    char *rsp_code;
} SyncwerkPutrepoemailtokenProcPriv;

#define GET_PRIV(o)  \
   (G_TYPE_INSTANCE_GET_PRIVATE ((o), SYNCWERK_TYPE_PUTREPOEMAILTOKEN_PROC, SyncwerkPutrepoemailtokenProcPriv))

#define USE_PRIV \
    SyncwerkPutrepoemailtokenProcPriv *priv = GET_PRIV(processor);


G_DEFINE_TYPE (SyncwerkPutrepoemailtokenProc, syncwerk_putrepoemailtoken_proc, CCNET_TYPE_PROCESSOR)
...
release_resource(CcnetProcessor *processor)
{
    ...
    CCNET_PROCESSOR_CLASS (syncwerk_putrepoemailtoken_proc_parent_class)->release_resource (processor);
}


static void
syncwerk_putrepoemailtoken_proc_class_init (SyncwerkPutrepoemailtokenProcClass *klass)
{
    ...
    g_type_class_add_private (klass, sizeof (SyncwerkPutrepoemailtokenProcPriv));
}

static void
syncwerk_putrepoemailtoken_proc_init (SyncwerkPutrepoemailtokenProc *processor)
{
}
...
get_repo_token (void *vprocessor)
{
    ...
    priv->rsp_code = SC_OK;
    if (!syncw_repo_manager_repo_exists (syncw->repo_mgr, priv->repo_id)) {
        priv->rsp_code = SC_NO_REPO;

    } else {
        priv->token = syncw_repo_manager_get_repo_token_nonnull (
            syncw->repo_mgr, priv->repo_id, priv->email);
