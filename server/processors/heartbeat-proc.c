...
} SyncwerkHeartbeatProcPriv;

#define GET_PRIV(o)  \
   (G_TYPE_INSTANCE_GET_PRIVATE ((o), SYNCWERK_TYPE_HEARTBEAT_PROC, SyncwerkHeartbeatProcPriv))

#define USE_PRIV \
    SyncwerkHeartbeatProcPriv *priv = GET_PRIV(processor);


G_DEFINE_TYPE (SyncwerkHeartbeatProc, syncwerk_heartbeat_proc, CCNET_TYPE_PROCESSOR)
...
static void collect_block_sizes_done (CcnetProcessor *processor,
                                      int status,
                                      char *message);
static void on_block_added (SyncwBlockManager *block_mgr,
                            const char *block_id,
                            void *vprocessor);
...
release_resource(CcnetProcessor *processor)
{
    ...
    g_signal_handlers_disconnect_by_func (syncw->block_mgr, on_block_added, processor);
    ccnet_timer_free (&priv->flush_timer);

    CCNET_PROCESSOR_CLASS (syncwerk_heartbeat_proc_parent_class)->release_resource (processor);
}


static void
syncwerk_heartbeat_proc_class_init (SyncwerkHeartbeatProcClass *klass)
{
    ...
    g_type_class_add_private (klass, sizeof (SyncwerkHeartbeatProcPriv));
}

static void
syncwerk_heartbeat_proc_init (SyncwerkHeartbeatProc *processor)
{
}
...
start (CcnetProcessor *processor, int argc, char **argv)
{
    ...
    snprintf (buf, sizeof(buf), "remote %s syncwerk-heartbeat", 
              processor->peer_id);
               ...
               char *content, int clen)
{
    ...
    if (strncmp (code, SC_OK, 3) == 0) {
        ...
        g_signal_connect (syncw->block_mgr, "block-added",
...
collect_block_sizes (void *vprocessor)
{
    ...
    metadata_list = syncw_block_manager_get_all_block_metadata (syncw->block_mgr);
    /* if (!metadata_list) { */
...
static void 
on_block_added (SyncwBlockManager *block_mgr,
                const char *block_id,
                void *vprocessor)
{
    CcnetProcessor *processor = vprocessor;
    ...
    md = syncw_block_manager_stat_block (block_mgr, block_id);
    if (!md) {
