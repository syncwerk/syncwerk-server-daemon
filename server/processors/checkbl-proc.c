...
typedef struct  {
    ...
    char        store_id[37];
    int         repo_version;
} SyncwerkCheckblProcPriv;

#define GET_PRIV(o)  \
   (G_TYPE_INSTANCE_GET_PRIVATE ((o), SYNCWERK_TYPE_CHECKBL_PROC, SyncwerkCheckblProcPriv))

#define USE_PRIV \
    SyncwerkCheckblProcPriv *priv = GET_PRIV(processor);


G_DEFINE_TYPE (SyncwerkCheckblProc, syncwerk_checkbl_proc, CCNET_TYPE_PROCESSOR)
...
release_resource(CcnetProcessor *processor)
{
    ...
    CCNET_PROCESSOR_CLASS (syncwerk_checkbl_proc_parent_class)->release_resource (processor);
}


static void
syncwerk_checkbl_proc_class_init (SyncwerkCheckblProcClass *klass)
{
    ...
    g_type_class_add_private (klass, sizeof (SyncwerkCheckblProcPriv));
}

static void
syncwerk_checkbl_proc_init (SyncwerkCheckblProc *processor)
{
}
               ...
               char *content, int clen)
{
    ...
    if (memcmp (code, SC_BLOCK_LIST, 3) == 0) {
        ...
        if (clen == 0 || clen % 40 != 0) {
            syncw_warning ("Bad block list length %d.\n", priv->len);
            ccnet_processor_send_response (processor, SC_SHUTDOWN, SS_SHUTDOWN,
                                           NULL, 0);
            ccnet_processor_done (processor, FALSE);
            return;
