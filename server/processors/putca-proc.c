...
#define GET_PRIV(o)  \
   (G_TYPE_INSTANCE_GET_PRIVATE ((o), SYNCWERK_TYPE_PUTCA_PROC, SyncwerkPutcaProcPriv))

#define USE_PRIV \
    SyncwerkPutcaProcPriv *priv = GET_PRIV(processor);
...
G_DEFINE_TYPE (SyncwerkPutcaProc, syncwerk_putca_proc, CCNET_TYPE_PROCESSOR)
...
release_resource (CcnetProcessor *processor)
{
    ...
    CCNET_PROCESSOR_CLASS (syncwerk_putca_proc_parent_class)->release_resource (processor);
}

static void
syncwerk_putca_proc_class_init (SyncwerkPutcaProcClass *klass)
{
    ...
    g_type_class_add_private (klass, sizeof (SyncwerkPutcaProcPriv));
}

static void
syncwerk_putca_proc_init (SyncwerkPutcaProc *processor)
{
}
...
check_token_thread (void *vdata)
{
    ...
    user = syncw_repo_manager_get_email_by_token (
        syncw->repo_mgr, priv->repo_id, priv->token);
    if (!user) {
...
process_commit_id_list (CcnetProcessor *processor, char *content, int clen)
{
    ...
    if (size % 40 != 0) {
        syncw_warning ("Invalid commid id list size %d.\n", size);
...
static gboolean
find_common_ancestor (SyncwCommit *commit, void *data, gboolean *stop)
{
...
compute_common_ancestor_thread (void *vprocessor)
{
    ...
    if (syncw_commit_manager_traverse_commit_tree (syncw->commit_mgr,
                                                  ...
                                                  processor,
                                                  FALSE) < 0) {
        syncw_warning ("Failed to find common ancestor.\n");
