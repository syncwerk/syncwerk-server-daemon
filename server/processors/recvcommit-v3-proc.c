...
#define GET_PRIV(o)  \
   (G_TYPE_INSTANCE_GET_PRIVATE ((o), SYNCWERK_TYPE_RECVCOMMIT_V3_PROC, RecvcommitPriv))
...
G_DEFINE_TYPE (SyncwerkRecvcommitV3Proc, syncwerk_recvcommit_v3_proc, CCNET_TYPE_PROCESSOR)
...
release_resource (CcnetProcessor *processor)
{
    ...
    if (priv->registered)
        syncw_obj_store_unregister_async_write (syncw->commit_mgr->obj_store,
                                               priv->writer_id);

    CCNET_PROCESSOR_CLASS (syncwerk_recvcommit_v3_proc_parent_class)->release_resource (processor);
}

static void
syncwerk_recvcommit_v3_proc_class_init (SyncwerkRecvcommitV3ProcClass *klass)
{
...
static void
syncwerk_recvcommit_v3_proc_init (SyncwerkRecvcommitV3Proc *processor)
{
}
...
get_repo_info_done (void *data)
{
    ...
    if (priv->success) {
        ...
        priv->writer_id =
            syncw_obj_store_register_async_write (syncw->commit_mgr->obj_store,
                                                 priv->repo_id,
                                                 priv->repo_version,
                                                 write_done_cb,
                                                 processor);
...
write_done_cb (OSAsyncResult *res, void *cb_data)
{
    ...
    if (!res->success) {
        ccnet_processor_send_response (processor, SC_BAD_OBJECT, SS_BAD_OBJECT,
                                       NULL, 0);
        syncw_warning ("[recvcommit] Failed to write commit object.\n");
        ccnet_processor_done (processor, FALSE);
    } else {
...
save_commit (CcnetProcessor *processor, ObjectPack *pack, int len)
{
    ...
    return syncw_obj_store_async_write (syncw->commit_mgr->obj_store,
                                       priv->writer_id,
                                       pack->id,
                                       pack->object,
                                       len - 41,
...
receive_commit (CcnetProcessor *processor, char *content, int clen)
{
    ...
    if (clen < sizeof(ObjectPack)) {
        syncw_warning ("[recvcommit] invalid object id.\n");
        goto bad;
    }

    syncw_debug ("[recvcommit] recv commit object %.8s\n", pack->id);
                           ...
                           char *content, int clen)
{
    ...
    case RECV_OBJECT:
        if (strncmp(code, SC_OBJECT, 3) == 0) {
            ...
            syncw_debug ("[recvcommit] Recv commit end.\n");
            ccnet_processor_done (processor, TRUE);
        } else {
            syncw_warning ("[recvcommit] Bad update: %s %s\n", code, code_msg);
            ccnet_processor_send_response (processor,
