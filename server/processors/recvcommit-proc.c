...
typedef struct  {
    char        object_path[SYNCW_PATH_MAX];
    char        tmp_object_path[SYNCW_PATH_MAX];
    char        buf[4096];
    char       *bufptr;
    int         pending_objects;
} SyncwerkRecvcommitProcPriv;

#define GET_PRIV(o)  \
   (G_TYPE_INSTANCE_GET_PRIVATE ((o), SYNCWERK_TYPE_RECVCOMMIT_PROC, SyncwerkRecvcommitProcPriv))

#define USE_PRIV \
    SyncwerkRecvcommitProcPriv *priv = GET_PRIV(processor);
...
G_DEFINE_TYPE (SyncwerkRecvcommitProc, syncwerk_recvcommit_proc, CCNET_TYPE_PROCESSOR)

static void
syncwerk_recvcommit_proc_class_init (SyncwerkRecvcommitProcClass *klass)
{
    ...
    g_type_class_add_private (klass, sizeof (SyncwerkRecvcommitProcPriv));
}

static void
syncwerk_recvcommit_proc_init (SyncwerkRecvcommitProc *processor)
{
}

inline static void
request_object_batch_begin (SyncwerkRecvcommitProcPriv *priv)
{
    priv->bufptr = priv->buf;
}

inline static void
request_object_batch (SyncwerkRecvcommitProcPriv *priv, const char *id)
{
...
request_object_batch_flush (CcnetProcessor *processor,
                            SyncwerkRecvcommitProcPriv *priv)
{
...
check_commit (CcnetProcessor *processor, const char *commit_id)
{
    ...
    if (!syncw_commit_manager_commit_exists (syncw->commit_mgr, commit_id)) {
...
{
    return syncw_obj_store_write_obj (syncw->commit_mgr->obj_store,
                                     pack->id,
                                     pack->object,
                                     len - 41);
}
...
bad:
    ...
    syncw_warning ("Bad commit object received.\n");
    ccnet_processor_done (processor, FALSE);
}
...
process_commit_list (CcnetProcessor *processor, char *content, int clen)
{
    ...
    if (clen % 41 != 1 || content[clen-1] != '\0') {
        syncw_warning ("Bad commit id list.\n");
