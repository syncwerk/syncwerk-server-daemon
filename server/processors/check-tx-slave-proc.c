...
typedef struct {
    ...
    char head_id[41];
    int has_branch;
} SyncwerkCheckTxSlaveProcPriv;

#define GET_PRIV(o)  \
   (G_TYPE_INSTANCE_GET_PRIVATE ((o), SYNCWERK_TYPE_CHECK_TX_SLAVE_PROC, SyncwerkCheckTxSlaveProcPriv))

#define USE_PRIV \
    SyncwerkCheckTxSlaveProcPriv *priv = GET_PRIV(processor);

G_DEFINE_TYPE (SyncwerkCheckTxSlaveProc, syncwerk_check_tx_slave_proc, CCNET_TYPE_PROCESSOR)
...
release_resource(CcnetProcessor *processor)
{
    ...
    CCNET_PROCESSOR_CLASS (syncwerk_check_tx_slave_proc_parent_class)->release_resource (processor);
}


static void
syncwerk_check_tx_slave_proc_class_init (SyncwerkCheckTxSlaveProcClass *klass)
{
    ...
    g_type_class_add_private (klass, sizeof (SyncwerkCheckTxSlaveProcPriv));
}

static void
syncwerk_check_tx_slave_proc_init (SyncwerkCheckTxSlaveProc *processor)
{
}
...
static gboolean
check_repo_share_permission (RpcsyncwerkClient *rpc_client,
                             const char *repo_id,
                             const char *user_name)
{
    ...
    if (syncw_share_manager_check_permission (syncw->share_mgr,
                                             repo_id,
                                             user_name) != NULL)
        return TRUE;
    ...
    for (pgroup = groups; pgroup != NULL; pgroup = pgroup->next) {
        ...
        repos = syncw_repo_manager_get_group_repoids (syncw->repo_mgr,
                                                     group_id, NULL);
...
check_tx (void *vprocessor)
{
    ...
    rpc_client = create_sync_ccnetrpc_client
        (syncw->session->central_config_dir, syncw->session->config_dir, "ccnet-threaded-rpcserver");
    ...
    owner = syncw_repo_manager_get_repo_owner (syncw->repo_mgr, repo_id);
    if (owner != NULL) {
        ...
        org_id = syncw_repo_manager_get_repo_org (syncw->repo_mgr, repo_id);
        if (org_id < 0 ||
...
get_email_cb (void *result, void *data, GError *error)
{
    ...
    if (!email) {
        syncw_warning ("[check tx] cannot find email for peer %s.\n",
                   processor->peer_id);
        ccnet_processor_send_response (processor, 
...
start (CcnetProcessor *processor, int argc, char **argv)
{
    ...
    ccnet_get_binding_email_async (syncw->async_ccnetrpc_client_t, processor->peer_id,
