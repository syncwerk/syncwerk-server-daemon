...
typedef struct {
    ...
    char *rsp_msg;
    char commit_id[41];
} SyncwerkSyncRepoSlaveProcPriv;

#define GET_PRIV(o)  \
   (G_TYPE_INSTANCE_GET_PRIVATE ((o), SYNCWERK_TYPE_SYNC_REPO_SLAVE_PROC, SyncwerkSyncRepoSlaveProcPriv))

#define USE_PRIV \
    SyncwerkSyncRepoSlaveProcPriv *priv = GET_PRIV(processor);

G_DEFINE_TYPE (SyncwerkSynRepoSlaveProc, syncwerk_sync_repo_slave_proc, CCNET_TYPE_PROCESSOR)
...
release_resource(CcnetProcessor *processor)
{
    ...
    CCNET_PROCESSOR_CLASS (syncwerk_sync_repo_slave_proc_parent_class)->release_resource (processor);
}

static void
syncwerk_sync_repo_slave_proc_class_init (SyncwerkSynRepoSlaveProcClass *klass)
{
    CcnetProcessorClass *proc_class = CCNET_PROCESSOR_CLASS (klass);

    proc_class->name = "syncwerk-sync-repo-slave-proc";
    proc_class->start = sync_repo_slave_start;
    proc_class->release_resource = release_resource;

    g_type_class_add_private (klass, sizeof (SyncwerkSyncRepoSlaveProcPriv));
}

static void
syncwerk_sync_repo_slave_proc_init (SyncwerkSynRepoSlaveProc *processor)
{
}
...
sync_repo_slave_start (CcnetProcessor *processor, int argc, char **argv)
{
    ...
    if (argc != 2) {
        syncw_warning ("[sync-repo-slave] argc(%d) must be 2\n", argc);
        ccnet_processor_done (processor, FALSE);
        return -1;
    }

    if (!is_uuid_valid(argv[0])) {
        syncw_warning ("Invalid repo_id %s.\n", argv[0]);
        ccnet_processor_done (processor, FALSE);
        return -1;
    }
    ...
    if (ccnet_processor_thread_create (processor, 
                                       ...
                                       thread_done,
                                       processor) < 0) {
        syncw_warning ("[sync repo] failed to start thread.\n");
...
send_repo_branch_info (void *vprocessor)                       
{
    ...
    if (syncwerk_server_db_statement_foreach_row (syncw->db, sql, 
        ...
        syncw_warning ("DB error when get branch %s.\n", priv->branch_name);
