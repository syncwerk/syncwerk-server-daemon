...
IndexBlksMgr *
index_blocks_mgr_new (SyncwerkSession *session)
{
    GError *error = NULL;
    ...
    if (!priv->idx_tpool) {
        if (error) {
            syncw_warning ("Failed to create index task thread pool: %s.\n", error->message);
            g_clear_error (&error);
        } else {
            syncw_warning ("Failed to create index task thread pool.\n");
        }
        g_free (priv);
        g_free (mgr);
        return NULL;
...
free_index_para (IndexPara *idx_para)
{
    ...
    syncw_repo_unref (idx_para->repo);
...
start_index_task (gpointer data, gpointer user_data)
{
    ...
    SyncwerkCrypt *crypt = idx_para->crypt;
                                 ...
                                 GError **error)
{
    ...
    if (!progress) {
        syncw_warning ("Index progress not found for token %s\n", token);
                              ...
                              char **task_id)
{
    ...
    if (!token) {
        syncw_warning ("Failed to genarate index token for repo %.8s.\n", repo_id);
        syncw_repo_unref (repo);
        return -1;
    }
    if (crypt) {
        _crypt = g_new0(SyncwerkCrypt, 1);
        memcpy (_crypt, crypt, sizeof (SyncwerkCrypt));
    }
