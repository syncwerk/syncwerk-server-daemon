...
{
    SyncwCopyManager *mgr = g_new0 (SyncwCopyManager, 1);

    mgr->session = session;
    mgr->priv = g_new0 (struct _SyncwCopyManagerPriv, 1);
...
syncw_copy_manager_get_task (SyncwCopyManager *mgr,
                            const char *task_id)
{
    SyncwCopyManagerPriv *priv = mgr->priv;
    CopyTask *task;
    SyncwerkCopyTask *t = NULL;

    pthread_mutex_lock (&priv->lock);

    task = g_hash_table_lookup (priv->copy_tasks, task_id);
    if (task) {
        t = syncwerk_copy_task_new ();
