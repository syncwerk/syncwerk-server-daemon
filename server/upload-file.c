...
#define DEBUG_FLAG SYNCWERK_DEBUG_HTTP
#include "log.h"
...
redirect_to_upload_error (evhtp_request_t *req,
                          ...
                          int error_code)
{
    char *restapi_url, *escaped_path, *escaped_fn = NULL;
    char url[1024];

    restapi_url = syncw->session->base.service_url;
    ...
    if (filename) {
        ...
        snprintf(url, 1024, "%s/repo/upload_error/%s?p=%s&fn=%s&err=%d",
                 restapi_url, repo_id, escaped_path, escaped_fn, error_code);
    } else {
        snprintf(url, 1024, "%s/repo/upload_error/%s?p=%s&err=%d",
                 restapi_url, repo_id, escaped_path, error_code);
    }
    g_free (escaped_path);
    g_free (escaped_fn);
                          ...
                          const char *parent_dir)
{
    ...
    restapi_url = syncw->session->base.service_url;
    escaped_path = g_uri_escape_string (parent_dir, NULL, FALSE);
    snprintf(url, 1024, "%s/repo/%s?p=%s", restapi_url, repo_id, escaped_path);
    g_free (escaped_path);
...
check_tmp_file_list (GList *tmp_files, int *error_code)
{
    ...
    for (ptr = tmp_files; ptr; ptr = ptr->next) {
        ...
        if (syncw_stat (tmp_file, &st) < 0) {
            syncw_warning ("[upload] Failed to stat temp file %s.\n", tmp_file);
    ...
    /* default is MB */
    max_upload_size = syncw_cfg_manager_get_config_int64 (syncw->cfg_mgr, "fileserver",
                                                         "max_upload_size");
    ...
    if (max_upload_size > 0 && total_size > max_upload_size) {
        syncw_debug ("[upload] File size is too large.\n");
                  ...
                  const char *parent_dir)
{
    ...
    if (!repo) {
        syncw_warning ("[upload] Failed to get repo %.8s.\n", repo_id);
    ...
    commit = syncw_commit_manager_get_commit (syncw->commit_mgr,
                                             ...
                                             repo->head->commit_id);
    if (!commit) {
        syncw_warning ("[upload] Failed to get head commit for repo %.8s.\n", repo_id);
...
create_relative_path (RecvFSM *fsm, char *parent_dir, char **abs_path)
{
    ...
    if (relative_path != NULL && strcmp(relative_path, "") != 0) {
        rc = syncw_repo_manager_mkdir_with_parents (syncw->repo_mgr,
                                                   fsm->repo_id,
                                                   parent_dir,
                                                   relative_path,
                                                   fsm->user,
        ...
        if (rc < 0) {
            if (error) {
                syncw_warning ("[upload folder] %s.", error->message);
                g_clear_error (&error);
            }
            goto out;
        }

        *abs_path = g_new0 (char, SYNCW_PATH_MAX);
        if (!*abs_path) {
            rc = -1;
            goto out;
        }
...
upload_cb(evhtp_request_t *req, void *arg)
{
    ...
    if (!fsm->files) {
        syncw_debug ("[upload] No file uploaded.\n");
    ...
    if (!parent_dir) {
        syncw_debug ("[upload] No parent dir given.\n");
    ...
    if (syncw_quota_manager_check_quota_with_delta (syncw->quota_mgr,
                                                   fsm->repo_id,
                                                   content_len) != 0) {
    ...
    int rc = syncw_repo_manager_post_multi_files (syncw->repo_mgr,
                                                 fsm->repo_id,
                                                 parent_dir,
                                                 filenames_json,
                                                 tmp_files_json,
...
file_id_list_from_json (const char *ret_json)
{
    ...
    if (!array) {
        syncw_warning ("Failed to load ret_json: %s.\n", err.text);
        return NULL;
    }
...
upload_api_cb(evhtp_request_t *req, void *arg)
{
    ...
    if (replace_str) {
        ...
        if (replace != 0 && replace != 1) {
            syncw_debug ("[Upload] Invalid argument replace: %s.\n", replace_str);
    ...
    char *ret_json = NULL;
    char *task_id = NULL;
    rc = syncw_repo_manager_post_multi_files (syncw->repo_mgr,
                                             fsm->repo_id,
                                             parent_dir,
                                             filenames_json,
                                             tmp_files_json,
...
error:
    switch (error_code) {
    case ERROR_FILENAME:
        send_error_reply (req, SYNCW_HTTP_RES_BADFILENAME, "Invalid filename.\n");
        break;
    case ERROR_EXISTS:
        send_error_reply (req, SYNCW_HTTP_RES_EXISTS, "File already exists.\n");
        break;
    case ERROR_SIZE:
        send_error_reply (req, SYNCW_HTTP_RES_TOOLARGE, "File size is too large.\n");
        break;
    case ERROR_QUOTA:
        send_error_reply (req, SYNCW_HTTP_RES_NOQUOTA, "Out of quota.\n");
        break;
    case ERROR_RECV:
    case ERROR_INTERNAL:
...
upload_raw_blks_api_cb(evhtp_request_t *req, void *arg)
{
    ...
    int rc = syncw_repo_manager_post_blocks (syncw->repo_mgr,
                                            fsm->repo_id,
                                            blockids_json,
                                            tmp_files_json,
                                            fsm->user,
...
upload_blks_api_cb(evhtp_request_t *req, void *arg)
{
    ...
    if (replace_str) {
        replace = atoi(replace_str);
        if (replace != 0 && replace != 1) {
            syncw_debug ("[Upload-blks] Invalid argument replace: %s.\n", replace_str);
            send_error_reply (req, EVHTP_RES_BADREQ, "Invalid argument.\n");
            return;
        }
    }
    ...
    if (!file_name || !parent_dir || !size_str || file_size < 0) {
        syncw_debug ("[upload-blks] No parent dir or file name given.\n");
        send_error_reply (req, EVHTP_RES_BADREQ, "Invalid URL.\n");
        return;
    }
    if (!commitonly_str) {
    ...
    /*     rc = syncw_repo_manager_post_file_blocks (syncw->repo_mgr, */
    /*                                              fsm->repo_id, */
    /*                                              parent_dir, */
    /*                                              file_name, */
    /*                                              blockids_json, */
    ...
    if (blockids_json == NULL) {
        syncw_debug ("[upload-blks] No blockids given.\n");
        send_error_reply (req, EVHTP_RES_BADREQ, "Invalid URL.\n");
        return;
    }
    rc = syncw_repo_manager_commit_file_blocks (syncw->repo_mgr,
                                               fsm->repo_id,
                                               parent_dir,
                                               file_name,
                                               blockids_json,
...
error:
    ...
    case ERROR_BLOCK_MISSING:
        send_error_reply (req, SYNCW_HTTP_RES_BLOCK_MISSING, "Block missing.\n");
        break;
    case ERROR_RECV:
    case ERROR_INTERNAL:
        send_error_reply (req, EVHTP_RES_SERVERR, "Internal error.\n");
...
/*     int rc = syncw_repo_manager_post_file_blocks (syncw->repo_mgr, */
/*                                                  fsm->repo_id, */
/*                                                  parent_dir, */
/*                                                  file_name, */
/*                                                  blockids_json, */
...
update_cb(evhtp_request_t *req, void *arg)
{
    ...
    if (!fsm->files) {
        syncw_debug ("[update] No file uploaded.\n");
        send_error_reply (req, EVHTP_RES_BADREQ, "No file.\n");
        return;
    }
    ...
    if (!target_file) {
        syncw_debug ("[Update] No target file given.\n");
        send_error_reply (req, EVHTP_RES_BADREQ, "Invalid URL.\n");
        return;
    }
    ...
    int rc = syncw_repo_manager_put_file (syncw->repo_mgr,
                                         fsm->repo_id,
                                         (char *)(fsm->files->data),
                                         parent_dir,
                                         filename,
...
error:
    ...
    case ERROR_NOT_EXIST:
        send_error_reply (req, SYNCW_HTTP_RES_NOT_EXISTS, "File does not exist.\n");
        break;
    case ERROR_RECV:
    case ERROR_INTERNAL:
    default:
...
update_blks_api_cb(evhtp_request_t *req, void *arg)
{
    ...
    if (!target_file || !size_str || file_size < 0) {
        syncw_debug ("[Update-blks] No target file given.\n");
        send_error_reply (req, EVHTP_RES_BADREQ, "Invalid URL.\n");
        return;
    }
    commitonly_str = evhtp_kv_find (req->uri->query, "commitonly");
    ...
    /*     blockids_json = file_list_to_json (fsm->filenames); */
    /*     tmp_files_json = file_list_to_json (fsm->files); */
    /*     rc = syncw_repo_manager_put_file_blocks (syncw->repo_mgr, */
    /*                                             fsm->repo_id, */
    /*                                             parent_dir, */
    /*                                             filename, */
    /*                                             blockids_json, */
...
/*     blockids_json = file_list_to_json (fsm->filenames); */
/*     tmp_files_json = file_list_to_json (fsm->files); */
/*     int rc = syncw_repo_manager_put_file_blocks (syncw->repo_mgr, */
/*                                                 fsm->repo_id, */
/*                                                 parent_dir, */
/*                                                 filename, */
/*                                                 blockids_json, */
...
update_ajax_cb(evhtp_request_t *req, void *arg)
{
    ...
    if (syncw_stat (tmp_file_path, &st) < 0) {
        syncw_warning ("Failed to stat tmp file %s.\n", tmp_file_path);
        goto error;
    }
...
get_mime_header_param_value (const char *param)
{
    ...
    if (!first_quote || !last_quote || first_quote == last_quote) {
        syncw_debug ("[upload] Invalid mime param %s.\n", param);
        return NULL;
    }
...
parse_mime_header (char *header, RecvFSM *fsm)
{
    ...
    colon = strchr (header, ':');
    if (!colon) {
        syncw_debug ("[upload] bad mime header format.\n");
        return -1;
    }
    ...
    *colon = 0;
    if (strcmp (header, "Content-Disposition") == 0) {
        ...
        if (g_strv_length (params) < 2) {
            syncw_debug ("[upload] Too little params for mime header.\n");
            g_strfreev (params);
            return -1;
        }
        if (strcasecmp (params[0], "form-data") != 0) {
            syncw_debug ("[upload] Invalid Content-Disposition\n");
            g_strfreev (params);
            return -1;
        }
        ...
        if (!fsm->input_name) {
            syncw_debug ("[upload] No input-name given.\n");
            g_strfreev (params);
            return -1;
        }
        ...
        if (strcmp (fsm->input_name, "file") == 0) {
            ...
            for (p = params; *p != NULL; ++p) {
                if (strncasecmp (*p, "filename", strlen("filename")) == 0) {
                    ...
                    if (file_name) {
                        ...
                        if (!fsm->file_name)
                            syncw_debug ("File name is not valid utf8 encoding.\n");
                        g_free (file_name);
                    }
                    break;
                }
            ...
            if (!fsm->file_name) {
                syncw_debug ("[upload] No filename given.\n");
                g_strfreev (params);
                return -1;
            }
        }
...
open_temp_file (RecvFSM *fsm)
{
    ...
    g_string_printf (temp_file, "%s/%sXXXXXX",
                     syncw->http_server->http_temp_dir, base_name);
    g_free (base_name);
    ...
    *no_line = FALSE;
    ...
    if (line != NULL) {
        if (strstr (line, fsm->boundary) != NULL) {
            syncw_debug ("[upload] form field ends.\n");

            g_free (fsm->input_name);
            fsm->input_name = NULL;
            fsm->state = RECV_HEADERS;
        } else {
            syncw_debug ("[upload] form field is %s.\n", line);
    ...
    *no_line = FALSE;
    ...
    if (!line) {
        ...
        if (evbuffer_get_length (fsm->line) >= MAX_CONTENT_LINE) {
            syncw_debug ("[upload] recv file data %d bytes.\n",
                     evbuffer_get_length(fsm->line));
            if (fsm->recved_crlf) {
                if (writen (fsm->fd, "\r\n", 2) < 0) {
                    syncw_warning ("[upload] Failed to write temp file: %s.\n",
                               strerror(errno));
        ...
        *no_line = TRUE;
        ...
        syncw_debug ("[upload] file data ends.\n");
...
upload_read_cb (evhtp_request_t *req, evbuf_t *buf, void *arg)
{
    ...
    while (!no_line) {
        ...
        case RECV_INIT:
            line = evbuffer_readln (fsm->line, &len, EVBUFFER_EOL_CRLF_STRICT);
            if (line != NULL) {
                syncw_debug ("[upload] boundary line: %s.\n", line);
                if (!strstr (line, fsm->boundary)) {
                    syncw_debug ("[upload] no boundary found in the first line.\n");
                    free (line);
        ...
        case RECV_HEADERS:
            line = evbuffer_readln (fsm->line, &len, EVBUFFER_EOL_CRLF_STRICT);
            if (line != NULL) {
                syncw_debug ("[upload] mime header line: %s.\n", line);
                if (len == 0) {
                    /* Read an blank line, headers end. */
                    free (line);
                    if (g_strcmp0 (fsm->input_name, "file") == 0) {
                        if (open_temp_file (fsm) < 0) {
                            syncw_warning ("[upload] Failed open temp file, errno:[%d]\n", errno);
                            res = EVHTP_RES_SERVERR;
                            goto out;
                        }
                    }
                    syncw_debug ("[upload] Start to recv %s.\n", fsm->input_name);
...
get_http_header_param_value (const char *param)
{
    ...
    equal = strchr (param, '=');
    if (!equal) {
        syncw_debug ("[upload] Invalid http header param %s.\n", param);
        return NULL;
    }
...
get_boundary (evhtp_headers_t *hdr)
{
    ...
    if (!content_type) {
        syncw_debug ("[upload] Missing Content-Type header\n");
        return boundary;
    }
    ...
    if (!params || g_strv_length (params) < 2) {
        syncw_debug ("[upload] Too little params Content-Type header\n");
        g_strfreev (params);
        return boundary;
    }
    if (strcasecmp (params[0], "multipart/form-data") != 0) {
        syncw_debug ("[upload] Invalid Content-Type\n");
        g_strfreev (params);
        return boundary;
    }
    ...
    g_strfreev (params);
    if (!boundary) {
        syncw_debug ("[upload] boundary not given\n");
    }
                    ...
                    char **token_type)
{
    ...
    webaccess = (SyncwerkWebAccess *)
        syncw_web_at_manager_query_access_token (syncw->web_at_mgr, token);
    if (!webaccess)
        return -1;
     ...
     */
    op = syncwerk_web_access_get_op (webaccess);
    if (token_type)
    ...
    *repo_id = g_strdup (syncwerk_web_access_get_repo_id (webaccess));
    *user = g_strdup (syncwerk_web_access_get_username (webaccess));
                   ...
                   char **progress_id)
{
    ...
    *progress_id = g_strdup(uuid);
    ...
    if (!content_len_str) {
        syncw_debug ("[upload] Content-Length not found.\n");
        return -1;
    }
...
upload_headers_cb (evhtp_request_t *req, evhtp_headers_t *hdr, void *arg)
{
    ...
    if (!token) {
        syncw_debug ("[upload] No token in url.\n");
        err_msg = "Invalid URL";
        goto err;
    }
...
idx_progress_cb(evhtp_request_t *req, void *arg)
{
    ...
    if (!progress_id) {
        syncw_debug ("[get pg] Index task id not found in url.\n");
        send_error_reply (req, EVHTP_RES_BADREQ, "task id not found");
        return;
    }
    char *progress_info = index_blocks_mgr_query_progress (syncw->index_blocks_mgr,
                                                           progress_id, NULL);
    if (!progress_info) {
...
upload_progress_cb(evhtp_request_t *req, void *arg)
{
    ...
    if (!progress_id) {
        syncw_debug ("[get pg] Progress id not found in url.\n");
    ...
    if (!callback) {
        syncw_debug ("[get pg] callback not found in url.\n");
    ...
    if (!progress) {
        /* syncw_warning ("[get pg] No progress found for %s.\n", progress_id); */
...
upload_file_init (evhtp_t *htp, const char *http_temp_dir)
{
    ...
    if (g_mkdir_with_parents (http_temp_dir, 0777) < 0) {
        syncw_warning ("Failed to create temp file dir %s.\n",
                      http_temp_dir);
        return -1;
    }
