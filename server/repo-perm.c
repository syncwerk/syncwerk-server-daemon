...
static gboolean
check_repo_share_perm_cb (SyncwDBRow *row, void *data)
{
    char **orig_perm = data;
    char *perm = g_strdup (syncwerk_server_db_row_get_column_text (row, 0));
...
static char *
check_group_permission_by_user (SyncwRepoManager *mgr,
                                const char *repo_id,
                                const char *user_name)
{
    ...
    if (syncwerk_server_db_statement_foreach_row (mgr->syncw->db, sql->str,
                                       check_repo_share_perm_cb, &permission,
                                       1, "string", repo_id) < 0) {
        syncw_warning ("DB error when get repo share permission for repo %s.\n", repo_id);
    }
...
static char *
check_repo_share_permission (SyncwRepoManager *mgr,
                             const char *repo_id,
                             const char *user_name)
{
    char *permission;

    permission = syncw_share_manager_check_permission (syncw->share_mgr,
                                                      repo_id,
                                                      user_name);
    if (permission != NULL)
        return permission;
    ...
    if (!mgr->syncw->cloud_mode)
        return syncw_repo_manager_get_inner_pub_repo_perm (mgr, repo_id);

    return NULL;
}

static char *
check_virtual_repo_permission (SyncwRepoManager *mgr,
                               const char *repo_id,
                               ...
                               GError **error)
{
    ...
    owner = syncw_repo_manager_get_repo_owner (mgr, origin_repo_id);
 ...
 */
char *
syncw_repo_manager_check_permission (SyncwRepoManager *mgr,
                                    const char *repo_id,
                                    const char *user,
                                    GError **error)
{
...
GList *
syncw_repo_manager_list_dir_with_perm (SyncwRepoManager *mgr,
                                      const char *repo_id,
                                      const char *dir_path,
                                      const char *dir_id,
                                      const char *user,
                                      ...
                                      GError **error)
{
    ...
    perm = syncw_repo_manager_check_permission (mgr, repo_id, user, error);
    if (!perm) {
        if (*error == NULL)
            g_set_error (error, SYNCWERK_DOMAIN, SYNCW_ERR_BAD_ARGS, "Access denied");
        return NULL;
    }
    ...
    if (!repo->virtual_info) {
        char *repo_owner = syncw_repo_manager_get_repo_owner (syncw->repo_mgr, repo_id);
        if (repo_owner && strcmp (user, repo_owner) == 0) {
            shared_sub_dirs = syncw_share_manager_get_shared_sub_dirs (syncw->share_mgr,
                                                                      repo->store_id,
                                                                      dir_path);
        }
        g_free (repo_owner);
