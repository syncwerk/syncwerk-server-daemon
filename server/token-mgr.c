...
{
    SyncwTokenManager *mgr = g_new0(SyncwTokenManager, 1);
                                   ...
                                   const char *repo_id)
{
    ...
    g_string_append_printf (token, "%s\n%s\n%s\n%"G_GUINT64_FORMAT,
                            syncw->session->base.id,
                            client_id,
                            repo_id,
                            (guint64)time(NULL));

    /* Create signature with my private key. */
    sig_base64 = ccnet_sign_message (syncw->ccnetrpc_client, token->str);
                                 ...
                                 char *ret_repo_id)
{
    ...
    if (!rpc_client)
        rpc_client = syncw->ccnetrpc_client;
