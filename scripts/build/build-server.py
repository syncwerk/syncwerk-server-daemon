...
'''This script builds the syncwerk server tarball.
...
class Project(object):
    ...
    # Probject name, i.e. librpcsyncwerk/ccnet/syncwerk/restapi
    name = ''
    ...
    def __init__(self):
        ...
        self.prefix = os.path.join(conf[CONF_BUILDDIR], 'syncwerk-server', 'syncwerk')
...
class Restapi(Project):
    name = 'restapi'
    def __init__(self):
        Project.__init__(self)
        # nothing to do for restapi
        self.build_commands = [
        ]
    ...
    def write_version_to_settings_py(self):
        '''Write the version of current syncwerk server to restapi'''
        settings_py = os.path.join(self.projdir, 'restapi', 'settings.py')
...
def check_restapi_thirdpart(thirdpartdir):
    '''The ${thirdpartdir} must have django/djblets/gunicorn pre-installed. So
    we can copy it to restapi/thirdpart
...
def validate_args(usage, options):
    ...
    check_targz_src('restapi', syncwerk_version, srcdir)
    check_targz_src_no_version('syncwdav', srcdir)
    check_targz_src_no_version('objectstorage', srcdir)
    ...
    builddir = os.path.join(builddir, 'syncwerk-server-build')

    # [ thirdpartdir ]
    thirdpartdir = get_option(CONF_THIRDPARTDIR)
    check_restapi_thirdpart(thirdpartdir)
...
def show_build_info():
    ...
    info('------------------------------------------')
    info('Syncwerk server %s: BUILD INFO' % conf[CONF_VERSION])
    info('------------------------------------------')
...
def prepare_builddir(builddir):
    ...
    must_mkdir(os.path.join(builddir, 'syncwerk-server'))
    must_mkdir(os.path.join(builddir, 'syncwerk-server', 'syncwerk'))
...
def parse_args():
    ...
    parser.add_option(long_opt(CONF_THIRDPARTDIR),
                      ...
                      nargs=1,
                      help='where to find the thirdpart libs for restapi')
...
def setup_build_env():
    ...
    prefix = os.path.join(conf[CONF_BUILDDIR], 'syncwerk-server', 'syncwerk')
...
def copy_user_manuals():
    ...
    # src_pattern = os.path.join(builddir, Syncwerk().projdir, 'doc', '*.doc')
    src_pattern = os.path.join(builddir, Syncwerk().projdir, 'doc', 'syncwerk-tutorial.doc')
    dst_dir = os.path.join(builddir, 'syncwerk-server', 'syncwerk', 'docs')
...
def copy_syncwdav():
    dst_dir = os.path.join(conf[CONF_BUILDDIR], 'syncwerk-server', 'restapi', 'thirdpart')
    tarball = os.path.join(conf[CONF_SRCDIR], 'syncwdav.tar.gz')
    if run('tar xf %s -C %s' % (tarball, dst_dir)) != 0:
        error('failed to uncompress %s' % tarball)

    dst_dir = os.path.join(conf[CONF_BUILDDIR], 'syncwerk-server', 'restapi', 'thirdpart')
    tarball = os.path.join(conf[CONF_SRCDIR], 'objectstorage.tar.gz')
    if run('tar xf %s -C %s' % (tarball, dst_dir)) != 0:
        error('failed to uncompress %s' % tarball)

def copy_scripts_and_libs():
    '''Copy server release scripts and shared libs, as well as restapi
    thirdpart libs

    '''
    builddir = conf[CONF_BUILDDIR]
    scripts_srcdir = os.path.join(builddir, Syncwerk().projdir, 'scripts')
    serverdir = os.path.join(builddir, 'syncwerk-server')

    must_copy(os.path.join(scripts_srcdir, 'setup-syncwerk.sh'),
              serverdir)
    must_copy(os.path.join(scripts_srcdir, 'setup-syncwerk-mysql.sh'),
              serverdir)
    must_copy(os.path.join(scripts_srcdir, 'setup-syncwerk-mysql.py'),
              serverdir)
    must_copy(os.path.join(scripts_srcdir, 'syncwerk.sh'),
              serverdir)
    must_copy(os.path.join(scripts_srcdir, 'restapi.sh'),
              serverdir)
    must_copy(os.path.join(scripts_srcdir, 'reset-admin.sh'),
              serverdir)
    must_copy(os.path.join(scripts_srcdir, 'syncwerk-server-fuse.sh'),
              serverdir)
    must_copy(os.path.join(scripts_srcdir, 'check_init_admin.py'),
              serverdir)
    must_copy(os.path.join(scripts_srcdir, 'syncwerk-server-gc.sh'),
              serverdir)
    must_copy(os.path.join(scripts_srcdir, 'syncwerk-server-fsck.sh'),
              serverdir)
    ...
    # copy runtime/restapi.conf
    runtimedir = os.path.join(serverdir, 'runtime')
    must_mkdir(runtimedir)
    must_copy(os.path.join(scripts_srcdir, 'restapi.conf'),
              runtimedir)

    # move restapi to syncwerk-server/restapi
    src_restapidir = Restapi().projdir
    dst_restapidir = os.path.join(serverdir, 'restapi')
    try:
        shutil.move(src_restapidir, dst_restapidir)
    except Exception, e:
        error('failed to move restapi to syncwerk-server/restapi: %s' % e)

    # copy restapi thirdpart libs
    restapi_thirdpart = os.path.join(dst_restapidir, 'thirdpart')
    copy_restapi_thirdpart_libs(restapi_thirdpart)
    copy_syncwdav()
...
def get_dependent_libs(executable):
    syslibs = ['librpcsyncwerk', 'libccnet', 'libsyncwerk', 'libpthread.so', 'libc.so', 'libm.so', 'librt.so', 'libdl.so', 'libselinux.so', 'libresolv.so' ]
    def is_syslib(lib):
...
def copy_shared_libs():
    ...
    syncwerk_path = os.path.join(builddir,
                                   'syncwerk-server',
                                   'syncwerk',
                                   'bin',
                                   'syncwerk-server-daemon')

    syncwerk_server_ccnet_path = os.path.join(builddir,
                                     'syncwerk-server',
                                     'syncwerk',
                                     'bin',
                                     'syncwerk-server-ccnet')

    syncwerk_server_fuse_path = os.path.join(builddir,
                                  'syncwerk-server',
                                  'syncwerk',
                                  'bin',
    ...
    libs = set()
    libs.update(get_dependent_libs(syncwerk_server_ccnet_path))
    libs.update(get_dependent_libs(syncwerk_path))
    libs.update(get_dependent_libs(syncwerk_server_fuse_path))
...
def copy_restapi_thirdpart_libs(restapi_thirdpart):
    ...
    try:
        ...
        error('failed to copy restapi thirdpart libs: %s' % e)
...
def strip_symbols():
    ...
    for parent, dnames, fnames in os.walk('syncwerk-server/syncwerk'):
...
def create_tarball(tarball_name):
    ...
    serverdir = 'syncwerk-server'
    versioned_serverdir = 'syncwerk-server-' + version

    # move syncwerk-server to syncwerk-server-${version}
    try:
    ...
    ignored_patterns = [
        ...
        # restapi
        os.path.join(versioned_serverdir, 'restapi', '.git*'),
        os.path.join(versioned_serverdir, 'restapi', 'media', 'flexpaper*'),
        os.path.join(versioned_serverdir, 'restapi', 'avatar', 'testdata*'),

        # syncwerk
        os.path.join(versioned_serverdir, 'syncwerk', 'share*'),
        os.path.join(versioned_serverdir, 'syncwerk', 'include*'),
        os.path.join(versioned_serverdir, 'syncwerk', 'lib', 'pkgconfig*'),
        os.path.join(versioned_serverdir, 'syncwerk', 'lib64', 'pkgconfig*'),
        os.path.join(versioned_serverdir, 'syncwerk', 'bin', 'ccnet-demo*'),
        os.path.join(versioned_serverdir, 'syncwerk', 'bin', 'ccnet-tool'),
        os.path.join(versioned_serverdir, 'syncwerk', 'bin', 'ccnet-servtool'),
        os.path.join(versioned_serverdir, 'syncwerk', 'bin', 'rpcsyncwerk-codegen.py'),
        os.path.join(versioned_serverdir, 'syncwerk', 'bin', 'syncwerk-admin'),
        os.path.join(versioned_serverdir, 'syncwerk', 'bin', 'syncwerk'),
    ]
...
def gen_tarball():
    ...
    # 64-bit: syncwerk-server_1.2.2_x86-64.tar.gz
    # 32-bit: syncwerk-server_1.2.2_i386.tar.gz
    version = conf[CONF_VERSION]
    ...
    tarball_name = 'syncwerk-server_%(version)s_%(arch)s%(dbg)s.tar.gz' \
