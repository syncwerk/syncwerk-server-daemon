...
'''This scirpt builds the syncwerk debian source tarball. In this tarball,
librpcsyncwerk and ccnet is also included.
...
def remove_unused_files():
    srcdir = os.path.join(conf[CONF_BUILDDIR], 'syncwerk-%s' % conf[CONF_VERSION])
...
def gen_tarball():
    output = os.path.join(conf[CONF_OUTPUTDIR], 'syncwerk-client-latest.tar.gz')
    dirname = 'syncwerk-%s' % conf[CONF_VERSION]
...
def uncompress_syncwerk():
    src = os.path.join(conf[CONF_BUILDDIR], 'syncwerk-%s' % conf[CONF_SYNCWERK_VERSION])
    dst = os.path.join(conf[CONF_BUILDDIR], 'syncwerk-%s' % conf[CONF_VERSION])
    ...
    tarball = os.path.join(conf[CONF_SRCDIR], 'syncwerk-%s.tar.gz' % conf[CONF_SYNCWERK_VERSION])
    argv = [ 'tar', 'xf',
             tarball,
    ...
    if run_argv(argv) != 0:
        error('failed to uncompress syncwerk')

    if conf[CONF_VERSION] != conf[CONF_SYNCWERK_VERSION]:
        shutil.move(src, dst)

def uncompress_librpcsyncwerk():
    tarball = os.path.join(conf[CONF_SRCDIR], 'librpcsyncwerk-%s.tar.gz' % conf[CONF_LIBRPCSYNCWERK_VERSION])
    dst_dir = os.path.join(conf[CONF_BUILDDIR], 'syncwerk-%s' % conf[CONF_VERSION], 'librpcsyncwerk')
    must_mkdir(dst_dir)
    argv = [ 'tar', 'xf',
             tarball,
             '--strip-components=1',
    ...
    if run_argv(argv) != 0:
        error('failed to uncompress librpcsyncwerk')

def uncompress_ccnet():
    tarball = os.path.join(conf[CONF_SRCDIR], 'ccnet-%s.tar.gz' % conf[CONF_CCNET_VERSION])
    dst_dir = os.path.join(conf[CONF_BUILDDIR], 'syncwerk-%s' % conf[CONF_VERSION], 'ccnet')
    must_mkdir(dst_dir)
    argv = [ 'tar', 'xf',
             tarball,
             '--strip-components=1',
...
def uncompress_syncwerk_client():
    tarball = os.path.join(conf[CONF_SRCDIR], 'syncwerk-client-%s.tar.gz' % conf[CONF_SYNCWERK_CLIENT_VERSION])
    dst_dir = os.path.join(conf[CONF_BUILDDIR], 'syncwerk-%s' % conf[CONF_VERSION], 'syncwerk-client')
    must_mkdir(dst_dir)
    argv = [ 'tar', 'xf',
             tarball,
             '--strip-components=1',
...
def remove_debian_subdir():
    debian_subdir = os.path.join(conf[CONF_BUILDDIR], 'syncwerk-%s' % conf[CONF_VERSION], 'debian')
...
def parse_args():
    ...
    parser.add_option(long_opt(CONF_VERSION),
                      dest=CONF_VERSION,
                      nargs=1,
                      help='the version of syncwerk source. Must be digits delimited by dots, like 1.3.0')

    parser.add_option(long_opt(CONF_SYNCWERK_VERSION),
                      dest=CONF_SYNCWERK_VERSION,
                      nargs=1,
                      help='the version of syncwerk. Must be digits delimited by dots, like 1.3.0')
...
def validate_args(usage, options):
    ...
    builddir = os.path.join(builddir, 'syncwerk-deb-src')
...
def show_build_info():
    ...
    info('------------------------------------------')
    info('Syncwerk debian source tarball %s:' % conf[CONF_VERSION])
    info('------------------------------------------')
...
def main():
    parse_args()
    uncompress_syncwerk()
    uncompress_librpcsyncwerk()
    uncompress_ccnet()
    uncompress_syncwerk_client()
