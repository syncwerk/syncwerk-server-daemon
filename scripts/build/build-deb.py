...
'''This scirpt builds the syncwerk server tarball.
...
def validate_args(usage, options):
    ...
    builddir = os.path.join(builddir, 'syncwerk-deb-build')
...
def show_build_info():
    ...
    info('------------------------------------------')
    info('Syncwerk debian package: BUILD INFO')
    info('------------------------------------------')
...
def setup_build_env():
    ...
    prefix = os.path.join(Syncwerk().projdir, 'debian', 'syncwerk', 'usr')
    ...
    if conf[CONF_NO_STRIP]:
        prepend_env_value('DEB_CPPFLAGS_APPEND',
                         '-g -O0',
                         seperator=' ')
        os.environ['SYNCWERK_NOSTRIP'] = 'true'
    ...
    os.environ['LIBRPCSYNCWERK_SOURCE_DIR'] = Librpcsyncwerk().projdir
    os.environ['CCNET_SOURCE_DIR'] = Ccnet().projdir
    os.environ['SYNCWERK_CLIENT_SOURCE_DIR'] = SyncwerkClient().projdir
