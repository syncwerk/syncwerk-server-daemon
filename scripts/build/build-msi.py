...
'''This scirpt builds the syncwerk windows msi installer.
...
2. When invoking commands like 'tar', we must convert the path to posix path with the function to_mingw_path. E.g., 'c:\\syncwerk' should be converted to '/c/syncwerk'.
...
CONF_SYNCWERK_CLIENT_VERSION  = 'syncwerk_client_version'
CONF_SRCDIR             = 'srcdir'
CONF_KEEP               = 'keep'
...
class SyncwerkClient(Project):
    name = 'syncwerk-client'
    def __init__(self):
        Project.__init__(self)
        ninja = find_in_path('ninja.exe')
        syncwerk_prefix = Syncwerk().prefix
        ...
        flags = {
            ...
            # which is in use
            'CMAKE_EXE_LINKER_FLAGS_RELEASE': '-L%s' % (os.path.join(syncwerk_prefix, 'lib') if ninja else to_mingw_path(os.path.join(syncwerk_prefix, 'lib'))),
        }
    ...
    def get_version(self):
        return conf[CONF_SYNCWERK_CLIENT_VERSION]
...
def validate_args(usage, options):
    ...
    syncwerk_client_version = get_option(CONF_SYNCWERK_CLIENT_VERSION)
    ...
    check_project_version(syncwerk_client_version)
    ...
    check_targz_src('syncwerk-client', syncwerk_client_version, srcdir)
    ...
    builddir = os.path.join(builddir, 'syncwerk-msi-build')
    ...
    conf[CONF_SYNCWERK_CLIENT_VERSION] = syncwerk_client_version
...
def show_build_info():
    ...
    info('------------------------------------------')
    info('Syncwerk msi installer: BUILD INFO')
    info('------------------------------------------')
    info('syncwerk:                  %s' % conf[CONF_VERSION])
    ...
    info('syncwerk-client:           %s' % conf[CONF_SYNCWERK_CLIENT_VERSION])
...
def parse_args():
    ...
    parser.add_option(long_opt(CONF_SYNCWERK_CLIENT_VERSION),
                      dest=CONF_SYNCWERK_CLIENT_VERSION,
                      nargs=1,
                      help='the version of syncwerk-client. Must be digits delimited by dots, like 1.3.0')
    ...
    parser.add_option(long_opt(CONF_QT5),
                      dest=CONF_QT5,
                      action='store_true',
                      help='''build syncwerk client with qt5''')

    parser.add_option(long_opt(CONF_WITH_SHIB),
                      dest=CONF_WITH_SHIB,
                      action='store_true',
                      help='''build syncwerk client with shibboleth support''')
...
def parse_depends_csv(path):
    ...
    libs = set()
    our_libs = ['librpcsyncwerk', 'libccnet', 'libsyncwerk']
...
def copy_shared_libs(exes):
    '''Copy shared libs need by syncwerk-applet.exe, such as libccnet,
    libsyncwerk, etc. First we use Dependency walker to analyse
    syncwerk-applet.exe, and get an output file in csv format. Then we parse
...
def copy_dll_exe():
    ...
    filelist = [
        os.path.join(prefix, 'bin', 'librpcsyncwerk-1.dll'),
        os.path.join(prefix, 'bin', 'libccnet-0.dll'),
        os.path.join(prefix, 'bin', 'libsyncwerk-0.dll'),
        os.path.join(prefix, 'bin', 'ccnet.exe'),
        os.path.join(prefix, 'bin', 'syncw-daemon.exe'),
        os.path.join(SyncwerkClient().projdir, 'syncwerk-applet.exe'),
    ]
    ...
    extdlls = [
        os.path.join(SyncwerkClient().projdir, 'extensions', 'lib', 'syncwerk_shell_ext.dll'),
        os.path.join(SyncwerkClient().projdir, 'extensions', 'lib', 'syncwerk_shell_ext64.dll'),
    ]
...
def copy_qt_translations():
    ...
    i18n_dir = os.path.join(SyncwerkClient().projdir, 'i18n')
    qm_pattern = os.path.join(i18n_dir, 'syncwerk_*.qm')
    ...
    for fn in glob.glob(qm_pattern):
        ...
        m = re.match(r'syncwerk_(.*)\.qm', name)
        lang = m.group(1)
        add_lang(lang)
...
def strip_symbols():
    ...
    for dll in glob.glob(os.path.join(bin_dir, '*.dll')):
        ...
        if 'qt' in name:
            do_strip(dll)
        if name == 'syncwerk_shell_ext.dll':
            do_strip(dll)
        elif name == 'syncwerk_shell_ext64.dll':
            do_strip(dll, stripcmd='x86_64-w64-mingw32-strip')

def edit_fragment_wxs():
    '''In the main wxs file(syncwerk.wxs) we need to reference to the id of
    syncwerk-applet.exe, which is listed in fragment.wxs. Since fragments.wxs is
    auto generated, the id is sequentially generated, so we need to change the
    id of syncwerk-applet.exe manually.
    ...
    with open(file_path, 'r') as fp:
        for line in fp:
            if 'syncwerk-applet.exe' in line:
                # change the id of 'syncwerk-applet.exe' to 'syncwerkapplet.exe'
                new_line = re.sub('file_bin_[\d]+', 'syncwerkapplet.exe', line)
...
def generate_breakpad_symbols():
    syncwerkdir = Syncwerk().projdir
    script = os.path.join(syncwerkdir, 'scripts/breakpad.py')
    symbol_file = 'syncw-daemon.exe.sym-%s' % conf[CONF_VERSION]
    output = os.path.join(syncwerkdir, symbol_file)
...
def build_msi():
    ...
    if run('make', cwd=pack_dir) != 0:
        error('Error when make syncwerk.msi')
...
def build_english_msi():
    ...
    if run('make en', cwd=pack_dir) != 0:
        error('Error when make syncwerk-en.msi')
...
def build_german_msi():
    ...
    if run('make de', cwd=pack_dir) != 0:
        error('Error when make syncwerk-de.msi')

def move_msi():
    pack_dir = os.path.join(conf[CONF_BUILDDIR], 'pack')
    src_msi = os.path.join(pack_dir, 'syncwerk.msi')
    ...
    if not conf[CONF_ONLY_CHINESE]:
        src_msi_en = os.path.join(pack_dir, 'syncwerk-en.msi')
        if not conf[CONF_WITH_SHIB]:
        ...
        src_msi_de = os.path.join(pack_dir, 'syncwerk-de.msi')
        if not conf[CONF_WITH_SHIB]:
...
def main():
    ...
    ccnet = Ccnet()
    syncwerk = Syncwerk()
    syncwerk_client = SyncwerkClient()
    ...
    syncwerk_client.uncompress()
    syncwerk_client.build()
