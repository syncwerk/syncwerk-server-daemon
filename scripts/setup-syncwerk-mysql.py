...
'''This script would guide the syncwerk admin to setup syncwerk with MySQL'''
...
class Utils(object):
    ...
    def welcome():
        ...
        welcome_msg = '''\
-----------------------------------------------------------------
This script will guide you to setup your syncwerk server using MySQL.
...
-----------------------------------------------------------------''' % SERVER_MANUAL_HTTP
    ...
    def get_python_executable():
        ...
        variable which is set in setup-syncwerk-mysql.sh
...
class EnvManager(object):
    ...
    def __init__(self):
        ...
        self.bin_dir = os.path.join(self.install_path, 'syncwerk', 'bin')
    ...
    def check_pre_condiction(self):
        ...
        paths = [
            os.path.join(self.install_path, 'syncwerk'),
            os.path.join(self.install_path, 'restapi'),
    ...
    def get_restapi_env(self):
        '''Prepare for restapi syncdb'''
        env = dict(os.environ)
        env['CCNET_CONF_DIR'] = ccnet_config.ccnet_dir
        env['SYNCWERK_CONF_DIR'] = syncwerk_config.syncwerk_dir
    ...
    def setup_python_path(self, env):
        ...
        extra_python_path = [
            ...
            os.path.join(install_path, 'restapi', 'thirdpart'),

            os.path.join(install_path, 'syncwerk/lib/python2.6/site-packages'),
            os.path.join(install_path, 'syncwerk/lib64/python2.6/site-packages'),
            os.path.join(install_path, 'syncwerk/lib/python2.7/site-packages'),
            os.path.join(install_path, 'syncwerk/lib64/python2.7/site-packages'),
        ]
    ...
    def get_binary_env(self):
        '''Set LD_LIBRARY_PATH for syncwerk server executables'''
        env = dict(os.environ)
        lib_dir = os.path.join(self.install_path, 'syncwerk', 'lib')
        lib64_dir = os.path.join(self.install_path, 'syncwerk', 'lib64')
...
class AbstractConfigurator(object):
    '''Abstract Base class for ccnet/syncwerk/restapi/db configurator'''
    def __init__(self):
        pass
...
class AbstractDBConfigurator(AbstractConfigurator):
    ...
    def __init__(self):
        ...
        self.syncwerk_mysql_user = ''
        self.syncwerk_mysql_password = ''
        self.syncwerk_mysql_userhost = 'localhost'

        self.root_password = ''
        self.root_conn = ''

        self.ccnet_db_name = ''
        self.syncwerk_db_name = ''
        self.restapi_db_name = ''

        self.restapi_admin_email = ''
        self.restapi_admin_password = ''
    ...
    def ask_use_existing_db():
        ...
        question = '''\
-------------------------------------------------------
Please choose a way to initialize syncwerk databases:
-------------------------------------------------------
'''

        note = '''\
[1] Create new ccnet/syncwerk/restapi databases
[2] Use existing ccnet/syncwerk/restapi databases
'''
    ...
    def ask_mysql_user_host(self):
        self.syncwerk_mysql_userhost = Utils.ask_question(
    ...
    def create_restapi_admin(self):
        try:
            conn = MySQLdb.connect(host=self.mysql_host,
                                   port=self.mysql_port,
                                   user=self.syncwerk_mysql_user,
                                   passwd=self.syncwerk_mysql_password,
...
CREATE TABLE IF NOT EXISTS EmailUser (id INTEGER NOT NULL PRIMARY KEY AUTO_INCREMENT, email VARCHAR(255), passwd CHAR(64), is_staff BOOL NOT NULL, is_active BOOL NOT NULL, ctime BIGINT, UNIQUE INDEX (email)) ENGINE=INNODB'''
        ...
        sql = '''REPLACE INTO EmailUser(email, passwd, is_staff, is_active, ctime) VALUES ('%s', '%s', 1, 1, 0)''' \
              % (restapi_config.admin_email, restapi_config.hashed_admin_password())
...
class NewDBConfigurator(AbstractDBConfigurator):
    '''Handles the case of creating new mysql databases for ccnet/syncwerk/restapi'''
    def __init__(self):
    ...
    def ask_questions(self):
        ...
        self.ask_syncwerk_mysql_user_password()

        self.ask_db_names()

    def generate(self):
        if not self.mysql_user_exists(self.syncwerk_mysql_user):
            self.create_user()
    ...
    def mysql_user_exists(self, user):
        ...
        sql = '''SELECT EXISTS(SELECT 1 FROM mysql.user WHERE user = '%s' and host = '%s')''' % \
                (user, self.syncwerk_mysql_userhost)
        ...
        try:
            ...
            if isinstance(e, MySQLdb.OperationalError):
                Utils.error('Failed to check mysql user %s@%s: %s' % \
                            (user, self.syncwerk_mysql_userhost, e.args[1]))
            else:
                Utils.error('Failed to check mysql user %s@%s: %s' % \
                            (user, self.syncwerk_mysql_userhost, e))
        finally:
            cursor.close()


    def ask_syncwerk_mysql_user_password(self):
        def validate(user):
            if user == 'root':
                raise InvalidAnswer(
                ...
                self.syncwerk_mysql_password = password

            return user


        question = 'Enter the name for mysql user of syncwerk. It would be created if not exists.'
        key = 'mysql user for syncwerk'
        default = 'syncwerk'
        self.syncwerk_mysql_user = Utils.ask_question(question,
                                                     key=key,
                                                     default=default,
                                                     validate=validate)
    ...
    def ask_db_names(self):
        self.ccnet_db_name = self.ask_db_name('syncwerk-server-ccnet', 'ccnet-db')
        self.syncwerk_db_name = self.ask_db_name('syncwerk-server', 'syncwerk-db')
        self.restapi_db_name = self.ask_db_name('restapi', 'restapi-db')
    ...
    def create_user(self):
        ...
        try:
            ...
            if isinstance(e, MySQLdb.OperationalError):
                Utils.error('Failed to create mysql user {}@{}: {}'.format(self.syncwerk_mysql_user, self.syncwerk_mysql_userhost, e.args[1]))
            else:
                Utils.error('Failed to create mysql user {}@{}: {}'.format(self.syncwerk_mysql_user, self.syncwerk_mysql_userhost, e))
        finally:
            cursor.close()
    ...
    def create_databases(self):
        ...
        self.create_db(self.syncwerk_db_name)
        self.create_db(self.restapi_db_name)

        if self.syncwerk_mysql_user != 'root':
            self.grant_db_permission(self.ccnet_db_name)
            self.grant_db_permission(self.syncwerk_db_name)
            self.grant_db_permission(self.restapi_db_name)


class ExistingDBConfigurator(AbstractDBConfigurator):
    '''Handles the case of use existing mysql databases for ccnet/syncwerk/restapi'''
    def __init__(self):
        AbstractDBConfigurator.__init__(self)
    ...
    def ask_questions(self):
        ...
        self.syncwerk_db_name = self.ask_db_name('syncwerk')
        self.restapi_db_name = self.ask_db_name('restapi')
    ...
    def ask_existing_mysql_user_password(self):
        ...
        question = 'Which mysql user to use for syncwerk?'
        key = 'mysql user for syncwerk'
        self.syncwerk_mysql_user = Utils.ask_question(question,
                                                     key=key,
                                                     validate=validate)
    ...
    def check_user_db_access(self, db_name):
        user = self.syncwerk_mysql_user
        password = self.syncwerk_mysql_password
...
class CcnetConfigurator(AbstractConfigurator):
    ...
    def generate_db_conf(self):
        ...
        config.set(db_section, 'USER', db_config.syncwerk_mysql_user)
        config.set(db_section, 'PASSWD', db_config.syncwerk_mysql_password)
...
class SyncwerkConfigurator(AbstractConfigurator):
    def __init__(self):
        AbstractConfigurator.__init__(self)
        self.syncwerk_dir = None
        self.port = 12001
        self.fileserver_port = None
        self.syncwerk_conf = os.path.join(env_mgr.central_config_dir, 'server.conf')

    def ask_questions(self):
        if not self.syncwerk_dir:
            self.ask_syncwerk_dir()
        # self.ask_port()
        if not self.fileserver_port:
            self.ask_fileserver_port()

    def generate(self):
        print 'Generating syncwerk configuration ...\n'
        syncwserv_init = os.path.join(env_mgr.bin_dir, 'syncwerk-server-daemon-init')
        argv = [
            syncwserv_init,
            '-F', env_mgr.central_config_dir,
            '--syncwerk-dir', self.syncwerk_dir,
        ...
        time.sleep(1)
        self.generate_db_conf()
        self.write_syncwerk_ini()
        print 'done'

    def generate_db_conf(self):
        config = Utils.read_config(self.syncwerk_conf)
        # [database]
        # type=
        # host=
        # user=
        ...
        config.set(db_section, 'port', db_config.mysql_port)
        config.set(db_section, 'user', db_config.syncwerk_mysql_user)
        config.set(db_section, 'password', db_config.syncwerk_mysql_password)
        config.set(db_section, 'db_name', db_config.syncwerk_db_name)
        config.set(db_section, 'connection_charset', 'utf8')

        Utils.write_config(config, self.syncwerk_conf)

    def validate_syncwerk_dir(self, path):
        if os.path.exists(path):
            raise InvalidAnswer('%s already exists' % Utils.highlight(path))
        return path

    def ask_syncwerk_dir(self):
        question = 'Where do you want to put your syncwerk data?'
        key = 'syncwerk-data'
        note = 'Please use a volume with enough free space'
        default = os.path.join(env_mgr.top_dir, 'syncwerk-data')
        self.syncwerk_dir = Utils.ask_question(question,
                                              key=key,
                                              note=note,
                                              default=default,
                                              validate=self.validate_syncwerk_dir)
    ...
    def write_syncwerk_ini(self):
        syncwerk_ini = os.path.join(ccnet_config.ccnet_dir, 'storage.ini')
        with open(syncwerk_ini, 'w') as fp:
            fp.write(self.syncwerk_dir)

class RestapiConfigurator(AbstractConfigurator):
    def __init__(self):
        AbstractConfigurator.__init__(self)
        self.admin_email = ''
        self.admin_password = ''
        self.restapi_settings_py = os.path.join(env_mgr.central_config_dir, 'restapi_settings.py')
    ...
    def generate(self):
        '''Generating restapi_settings.py'''
        print 'Generating restapi configuration ...\n'
        time.sleep(1)
        with open(self.restapi_settings_py, 'w') as fp:
    ...
    def write_secret_key(self, fp):
        script = os.path.join(env_mgr.install_path, 'restapi/tools/secret_key_generator.py')
        cmd = [
...
'''
        text = template % dict(name=db_config.restapi_db_name,
                               username=db_config.syncwerk_mysql_user,
                               password=db_config.syncwerk_mysql_password,
    ...
    def do_syncdb(self):
        print '----------------------------------------'
        print 'Now creating restapi database tables ...\n'
        print '----------------------------------------'

        try:
            conn = MySQLdb.connect(host=db_config.mysql_host,
                                   port=db_config.mysql_port,
                                   user=db_config.syncwerk_mysql_user,
                                   passwd=db_config.syncwerk_mysql_password,
                                   db=db_config.restapi_db_name)
        except Exception, e:
            if isinstance(e, MySQLdb.OperationalError):
                Utils.error('Failed to connect to mysql database %s: %s' % (db_config.restapi_db_name, e.args[1]))
            else:
                Utils.error('Failed to connect to mysql database %s: %s' % (db_config.restapi_db_name, e))

        cursor = conn.cursor()

        sql_file = os.path.join(env_mgr.install_path, 'restapi', 'sql', 'mysql.sql')
        ...
        for sql in sqls:
            try:
                ...
                if isinstance(e, MySQLdb.OperationalError):
                    Utils.error('Failed to init restapi database: %s' % e.args[1])
                else:
                    Utils.error('Failed to init restapi database: %s' % e)
    ...
    def prepare_avatar_dir(self):
        ...
        try:
            media_dir = os.path.join(env_mgr.install_path, 'restapi', 'media')
            orig_avatar_dir = os.path.join(media_dir, 'avatars')

            restapi_data_dir = os.path.join(env_mgr.top_dir, 'restapi-data')
            dest_avatar_dir = os.path.join(restapi_data_dir, 'avatars')

            if os.path.exists(dest_avatar_dir):
                return

            if not os.path.exists(restapi_data_dir):
                os.mkdir(restapi_data_dir)

            shutil.move(orig_avatar_dir, dest_avatar_dir)
            os.symlink('../../../restapi-data/avatars', orig_avatar_dir)
        except Exception, e:
            Utils.error('Failed to prepare restapi avatars dir: %s' % e)

class SyncwDavConfigurator(AbstractConfigurator):
    def __init__(self):
        AbstractConfigurator.__init__(self)
        self.syncwdav_conf = None

    def ask_questions(self):
        pass

    def generate(self):
        self.syncwdav_conf = os.path.join(env_mgr.central_config_dir, 'syncwdav.conf')
        text = '''
[WEBDAV]
enabled = false
port = 8080
...
'''
        ...
        with open(self.syncwdav_conf, 'w') as fp:
            fp.write(text)
...
class UserManualHandler(object):
    def __init__(self):
        self.src_docs_dir = os.path.join(env_mgr.install_path, 'syncwerk', 'docs')
        self.library_template_dir = None

    def copy_user_manuals(self):
        self.library_template_dir = os.path.join(syncwerk_config.syncwerk_dir, 'template')
...
def report_config():
    ...
    syncwerk data dir:       %(syncwerk_dir)s
    fileserver port:        %(fileserver_port)s

    database:               %(use_existing_db)s
    ccnet database:         %(ccnet_db_name)s
    syncwerk database:       %(syncwerk_db_name)s
    restapi database:        %(restapi_db_name)s
...
'''
    config = {
        ...
        'syncwerk_dir' :         syncwerk_config.syncwerk_dir,
        'fileserver_port' :     syncwerk_config.fileserver_port,

        'admin_email' :         restapi_config.admin_email,


        'use_existing_db':       'use existing' if db_config.use_existing_db else 'create new',
        'ccnet_db_name':        db_config.ccnet_db_name,
        'syncwerk_db_name':      db_config.syncwerk_db_name,
        'restapi_db_name':       db_config.restapi_db_name,
        'db_user':              db_config.syncwerk_mysql_user
    }
...
def create_syncwerk_server_symlink():
    print '\ncreating syncwerk-server-latest symbolic link ... ',
    syncwerk_server_symlink = os.path.join(env_mgr.top_dir, 'syncwerk-server-latest')
    try:
        os.symlink(os.path.basename(env_mgr.install_path), syncwerk_server_symlink)
    except Exception, e:
        print '\n'
        Utils.error('Failed to create symbolic link %s: %s' % (syncwerk_server_symlink, e))
    else:
        print 'done\n\n'
...
def set_file_perm():
    ...
    dirmode = 0700
    files = [
        restapi_config.restapi_settings_py,
    ]
    dirs = [
        env_mgr.central_config_dir,
        ccnet_config.ccnet_dir,
        syncwerk_config.syncwerk_dir,
        restapi_config.restapi_settings_py,
    ]
    for fpath in files:
...
syncwerk_config = SyncwerkConfigurator()
syncwdav_config = SyncwDavConfigurator()
gunicorn_config = GunicornConfigurator()
restapi_config = RestapiConfigurator()
...
def check_params(args):
    ...
    syncwerk_config.fileserver_port = Utils.validate_port(fileserver_port)

    syncwerk_dir = get_param_val(args.syncwerk_dir, 'SYNCWERK_DIR',
                                os.path.join(env_mgr.top_dir, 'syncwerk-data'))
    syncwerk_config.syncwerk_dir = syncwerk_config.validate_syncwerk_dir(syncwerk_dir)
    ...
    syncwerk_db = get_param_val(args.syncwerk_db, 'SYNCWERK_DB', 'syncwerk_db')
    if not syncwerk_db:
        raise InvalidParams('Incomplete mysql configuration parameters, ' \
                            'missing syncwerk db name parameter')

    restapi_db = get_param_val(args.restapi_db, 'RESTAPI_DB', 'restapi_db')
    if not restapi_db:
        raise InvalidParams('Incomplete mysql configuration parameters, ' \
                            'missing restapi db name parameter')
    ...
    if db_config.use_existing_db:
        ...
        db_config.syncwerk_mysql_user = mysql_user
        db_config.syncwerk_mysql_password = mysql_user_passwd
        db_config.ccnet_db_name = db_config.validate_db_name(ccnet_db)
        db_config.syncwerk_db_name = db_config.validate_db_name(syncwerk_db)
        db_config.restapi_db_name = db_config.validate_db_name(restapi_db)
    else:
        ...
        if not mysql_user_host:
            db_config.syncwerk_mysql_userhost = 'localhost'
        else:
            db_config.syncwerk_mysql_userhost = db_config.validate_mysql_user_host(mysql_user_host)
        ...
        if mysql_user == 'root':
            db_config.syncwerk_mysql_user = 'root'
            db_config.syncwerk_mysql_password = db_config.root_password
        else:
        ...
        db_config.syncwerk_db_name = syncwerk_db
        db_config.restapi_db_name = restapi_db
...
def main():
    if len(sys.argv) > 2 and sys.argv[1] == 'auto':
        ...
        parser.add_argument('-d', '--syncwerk-dir', help='syncwerk dir to store syncwerk data')
        ...
        parser.add_argument('-s', '--syncwerk-db', help='syncwerk db name')
        parser.add_argument('-b', '--restapi-db', help='restapi db name')
    ...
    syncwerk_config.ask_questions()
    restapi_config.ask_questions()
    ...
    syncwerk_config.generate()
    syncwdav_config.generate()
    gunicorn_config.generate()
    restapi_config.generate()

    restapi_config.do_syncdb()
    restapi_config.prepare_avatar_dir()
    # db_config.create_restapi_admin()
    user_manuals_handler.copy_user_manuals()
    create_syncwerk_server_symlink()
...
-----------------------------------------------------------------
Your syncwerk server configuration has been finished successfully.
-----------------------------------------------------------------

run syncwerk server:     ./syncwerk.sh { start | stop | restart }
run restapi  server:     ./restapi.sh  { start <port> | stop | restart <port> }

-----------------------------------------------------------------
If you are behind a firewall, remember to allow input/output of these tcp ports:
-----------------------------------------------------------------

port of syncwerk fileserver:   %(fileserver_port)s
port of restapi:               8000
...
'''
    ...
    print message % dict(fileserver_port=syncwerk_config.fileserver_port,
