...
The directory is ./symbols/syncw-daemon.exe/${symbol_id}/syncw-daemon.exe.sym,
...
def main():
    ...
    syncwerk_src_dir = dirname(abspath(dirname(__file__)))
    os.chdir(syncwerk_src_dir)
    program = 'syncw-daemon.exe' if os.name == 'nt' else 'syncw-daemon'
    syncw_daemon = join('daemon', '.libs', program)
    if not exists(syncw_daemon):
        syncw_daemon = join('daemon', program)
        if not exists(syncw_daemon):
            raise RuntimeError('syncw-daemon executable not found!')
    symbols = get_command_output('dump_syms {}'.format(syncw_daemon))
