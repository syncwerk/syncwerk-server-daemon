...
restapi_sql=${UPGRADE_DIR}/sql/2.0.0/sqlite3/restapi.sql
...
echo "fix syncwerk database case issues..."
if ! ${add_collate_script} ${default_ccnet_conf_dir} ${syncwerk_data_dir} ${restapi_db}; then
    echo "Failed."
    exit 1
fi
