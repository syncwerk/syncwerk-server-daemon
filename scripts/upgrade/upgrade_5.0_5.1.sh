...
export SYNCWERK_CENTRAL_CONF_DIR=${default_conf_dir}
...
function ensure_server_not_running() {
    ...
    if pgrep syncwerk-server-daemon 2>/dev/null 1>&2 ; then
        ...
        exit 1
         ...
         || pgrep -f "restapi.wsgi:application" 2>/dev/null 1>&2; then
        echo
...
# copy ccnet.conf/server.conf etc. to conf/ dir, and make the original files read-only
function copy_confs_to_central_conf_dir() {
    local confs=(
        $default_ccnet_conf_dir/ccnet.conf
        $syncwerk_data_dir/server.conf
        $restapi_settings_py
    )
    for conffile in ${confs[*]}; do
        ...
        cp $conffile $conffile.syncwerk-5.0.0-bak
        cp -av $conffile $default_conf_dir/
        cat >$conffile<<EOF
# This file has been moved to $default_conf_dir/$(basename $conffile) in syncwerk 5.0.0
EOF
        chmod a-w $conffile
    done
}
