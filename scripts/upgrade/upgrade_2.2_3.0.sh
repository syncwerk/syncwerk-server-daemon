...
function migrate_syncwerk_data_format() {
    syncwerk_server_migrate=${INSTALLPATH}/syncwerk/bin/syncwerk-server-migrate
    echo
    echo "now migrating syncwerk data to 3.0 format"
    echo
    if ! LD_LIBRARY_PATH=${SYNCWERK_LD_LIBRARY_PATH} ${syncwerk_server_migrate} \
            -c "${default_ccnet_conf_dir}" -d "${syncwerk_data_dir}"; then
        echo
        echo "Failed to migrate syncwerk data to 3.0 format"
        echo
        exit 1;
    fi
    echo
    echo "Successfully migrated syncwerk data to 3.0 format"
    echo
}
