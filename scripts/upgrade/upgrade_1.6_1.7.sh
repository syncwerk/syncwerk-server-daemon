...
restapi_sql=${UPGRADE_DIR}/sql/1.7.0/sqlite3/restapi.sql
if ! sqlite3 "${restapi_db}" < "${restapi_sql}"; then
    echo "Failed to update restapi database"
    exit 1
fi

syncwerk_db=${syncwerk_data_dir}/syncwerk.db
syncwerk_sql=${UPGRADE_DIR}/sql/1.7.0/sqlite3/syncwerk.sql
if ! sqlite3 "${syncwerk_db}" < "${syncwerk_sql}"; then
    echo "Failed to update syncwerk database"
    exit 1
fi
